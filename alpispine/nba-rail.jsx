    ...
    const items = React.useMemo(() => {
      ...
      if (!secured) {
        list.push({ id: "NBA-1", sev: "high", lens: "ops", screen: "flow", cta: "Review & sign off",
          title: "Sign off the re-accommodation", why: "3 booking writes are staged and reversible. The Bianchi 14-Aug date holds only once you release them — the allotment agent is paused on your decision." });
      } else {
        list.push({ id: "NBA-1b", sev: "info", lens: "ops", screen: "flow", cta: "View the sealed flow",
          title: "Re-accommodation committed", why: acked ? "Signed off and Rossi Travel accepted the secured plan — audit sealed. Nothing further needed." : "Signed off and committed. Awaiting Rossi Travel's confirmation on the customer side." });
      }
      ...
      if (samples.length) {
        list.push({ id: "NBA-S", sev: "med", lens: "ops", screen: "queue", cta: "Open planner queue",
          title: `Confirm ${samples.length} AlpiGPT hold${samples.length > 1 ? "s" : ""}`, why: `Rossi Travel requested ${samples.map(s => s.grade).join(", ")} via AlpiGPT — added to BK 88412 and waiting on a covenant check.` });
      }
      ...
      if (secured && !acked) {
        list.push({ id: "NBA-5", sev: "info", lens: "cx", screen: "customer", cta: "See customer view",
          title: "Rossi Travel hasn't confirmed yet", why: "The re-point is committed and the dates hold — nudge the agency to accept the secured plan from the customer lens." });
      }
