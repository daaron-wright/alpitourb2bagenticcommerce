...
const AL_ROLES = [
  ...
  ["Travel Assistant Orchestrator", "Interpret, clarify, retrieve, coordinate — never book unasked", "#8A4FBF"],
...
const AL_ROUNDS = [
  ["Round 1 · Inquiry → recommendation", "Request → advisor asks Travel Assistant → clarify & retrieve → confirms sold-out → alternatives ranked.", "Define: what new behaviours did we discover?"],
  ["Round 2 · Proposal, hold & date change", "Travel Assistant shapes the multi-city itinerary → assembles the proposal → advisor approves the hold → customer shifts dates → reprices to v2.", "Define: what should AI prepare, and what must the human approve?"],
  ["Round 3 · Ongoing Customer Support", "Mid-trip in the United Kingdom a train strike disrupts the London→Bath leg → Travel Assistant flags it → reworks transfers & a hotel night → advisor confirms with the family.", "Define: would this stay self-service, or would support be needed?"],
];
    ...
    <Board phase="enact" time="00:32–01:00" title="Role-play — the Future Travel Desk" output="Discovered behaviours, gaps & moments"
      ...
      <div className="al-card" style={{ flex: "0 0 auto", borderColor: "rgba(138,79,191,0.32)", background: "rgba(138,79,191,0.05)", flexDirection: "row", gap: 14, alignItems: "center", padding: "13px 18px" }}>
        ...
        <span style={{ fontSize: 14, lineHeight: 1.5, color: "var(--fg-1)" }}>
          A family of three from <Edit>Toronto, Canada</Edit> wants a <Edit>10-day</Edit> trip around <Edit>United Kingdom</Edit> in <Edit>August</Edit>, around <Edit>CA$11,000</Edit>. They love <Edit>food, history & slow mornings</Edit>, travel with a <Edit>2-year-old</Edit>, and hoped to take in <Edit>London, Bath & Cornwall</Edit>. Their non-negotiables: <Edit>family rooms, air-con & short transfers</Edit>. A few hotels are unavailable — recover the sale, assemble a proposal, place a hold, handle a date change, then support a <Edit>travel disruption mid-trip</Edit>.
        </span>
      </div>
...
const AL_CC_EX = {
  0: ["Clarifying questions · sold-out status · ranked United Kingdom alternatives", "Applies family / toddler signal · retrieves amenities · ranks family-fit options", "Sold-out request recovered · no support call"],
  ...
  2: ["Disruption alert · reworked transfers · added hotel night", "Detects the train strike · reworks the London→Bath leg · proposes a fix", "Traveler kept whole · cleaner escalation · operator load reduced"],
};
...
const AL_BP_EX = {
  0: ["Hoped for London, Bath & Cornwall", "Needs a credible alternative", "Travel Assistant clarifies & retrieves; detects sold-out; ranks family-fit options", "Inventory · product facts · pricing", "Advisor selects — no auto-switch", "Quote saved · ticket avoided"],
  ...
  2: ["Train strike hits the London→Bath leg", "Gets the fix to confirm with the family", "Detects the disruption; reworks transfers & adds a hotel night", "Live disruption · availability · rebooking", "Advisor approves the reworked plan", "Traveler kept whole · no ticket · traceable"],
};
