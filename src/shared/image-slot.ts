...
function save(): void {
  ...
  const w = (window as any).omelette && (window as any).omelette.writeFile;
  if (!w) return;
  saving = true;
...
function getSlot(id: string): { u?: string; s: number; x: number; y: number } | null {
  const v = slots[id];
  if (!v) return null;
  return typeof v === 'string' ? { u: v, s: 1, x: 0, y: 0 } : v;
}

function setSlot(id: string, val: any): void {
  if (!id) return;
...
async function toDataUrl(file: File, targetW: number): Promise<string> {
  ...
  try {
    ...
    (bitmap as any).close && (bitmap as any).close();
  }
}
...
class ImageSlot extends HTMLElement {
  ...
  _sub!: HTMLElement;
  _spill!: HTMLElement;
  _ghost!: HTMLImageElement;
  _err: HTMLElement | null = null;
  ...
  _subFn: () => void;
  _local: any = null;
  _userUrl: string | null = null;
  _ro: ResizeObserver | null = null;
  _outside: ((e: PointerEvent) => void) | null = null;
  _esc: ((e: KeyboardEvent) => void) | null = null;
  _dragUp: (() => void) | null = null;
  ...
  constructor() {
    ...
    this._spill.addEventListener('pointerdown', (e: PointerEvent) => {
      ...
      const corner = (e.target as Element).getAttribute && (e.target as Element).getAttribute('data-c');
  ...
  connectedCallback() {
    ...
    this.addEventListener('dragenter', this as any);
    this.addEventListener('dragover', this as any);
    this.addEventListener('dragleave', this as any);
    this.addEventListener('drop', this as any);
  ...
  disconnectedCallback() {
    ...
    this.removeEventListener('dragenter', this as any);
    this.removeEventListener('dragover', this as any);
    this.removeEventListener('dragleave', this as any);
    this.removeEventListener('drop', this as any);
  ...
  _enterReframe() {
    ...
    document.addEventListener('pointerdown', this._outside as any, true);
    document.addEventListener('keydown', this._esc as any, true);
  }

  _exitReframe(commit: boolean) {
    if (!this.hasAttribute('data-reframe')) return;
    if (this._dragUp) this._dragUp();
    this.removeAttribute('data-reframe');
    this.removeAttribute('data-panning');
    if (this._outside) document.removeEventListener('pointerdown', this._outside as any, true);
    if (this._esc) document.removeEventListener('keydown', this._esc as any, true);
  ...
  _commitView() {
    const v: any = { s: this._view.s, x: this._view.x, y: this._view.y };
  ...
  _render() {
    ...
    if (shape === 'circle') radius = '50%';
      ...
      const n = parseFloat(this.getAttribute('radius') || '12');
    ...
    const editable = !!((window as any).omelette && (window as any).omelette.writeFile);
