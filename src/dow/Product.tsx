...
export function Product({ id, onNav, onAskAgent, toast }: any) {
  const p = DX.productById(id) || DX.products[0];
  const docs = DX.docsFor(p), samples = DX.samplesFor(p);
  const related = DX.products.filter((x: any) => x.app === p.app && x.id !== p.id).concat(DX.products.filter((x: any) => x.app !== p.app)).slice(0, 3);
        ...
        <div className="dx-prodgrid2">
            ...
            {tab === "overview" && (
              <div className="dx-prose fade-in">
                ...
                <div className="dx-bullets">
                  {p.bullets.map((b: string, i: number) => <div className="dx-bullet" key={i}><Icon name="checkmark-filled" size={15} /> {b}</div>)}
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 8 }}>
                  {p.tags.map((t: string, i: number) => <span className="dx-pchip" key={i}>{t}</span>)}
                </div>
              </div>
            )}
            {tab === "props" && (
              <table className="dx-proptbl fade-in">
                ...
                <tbody>{p.props.map((r: any, i: number) => <tr key={i}><td>{r[0]}</td><td className="pv">{r[1]}</td><td>{r[2]}</td><td className="pm">{r[3]}</td></tr>)}</tbody>
              </table>
            )}
            {tab === "docs" && (
          ...
          <aside className="dx-side">
            <div className="dx-sidecard">
              <h4>Buying & access</h4>
              <p>Your {DX.account.company} account has sample access in {DX.account.region}. Production volumes are quoted per project.</p>
