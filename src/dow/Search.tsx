...
export function Search({ query, onNav, onSearch, onAskAgent }: any) {
  const r = resolve(query);
  const results = r.ids.map(DX.productById).filter(Boolean);
  return (
      ...
      <div className="dx-wrap">
        ...
        <div className="dx-srgrid">
          <aside className="dx-facets">
            {FACETS.map((f: any, i) => (
              <div key={i}><div className="fh">{f[0]}</div>{f[1].map((row: any, j: number) => <div className="dx-facet" key={j}><span className="cb" />{row[0]}<span className="fn">{row[1]}</span></div>)}</div>
            ))}
          </aside>
