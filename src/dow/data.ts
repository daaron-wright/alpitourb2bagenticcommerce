...
const docRef = (id: string) => documents[id] || { title: id, kind: "Doc", rev: "—", date: "", source: "internal", public: null };

const caseHistory: Record<string, any[]> = {
  "CASE-02111": [
...
const typeById = (id: string) => customerTypes.find(c => c.id === id);
...
const gateState: Record<string, any> = {
...
const dealFor = (code: string) => deals[code];
...
const docsFor = (p: any) => [["Technical Data Sheet (TDS)", "PDF \u00B7 240 KB", "EN"], ["Safety Data Sheet (SDS)", "PDF \u00B7 180 KB", "EN / multi"], [(p.app === "packaging" ? "EU 10/2011 declaration" : p.app === "auto" ? "Automotive compounding guide" : "Footwear foaming application guide"), "PDF \u00B7 1.2 MB", "EN"]];
const samplesFor = (_p: any) => [["200 g", "Lab evaluation", "In stock \u00B7 ships in 2 days"], ["2 kg", "Pilot trial", "In stock \u00B7 ships in 3 days"], ["10 kg", "Pre-production", "Lead time 5\u20137 days"]];
...
const caseByCode = (code: string) => cases.find(c => c.code === code);
...
const imageSteps: [string, string][] = [
...
const imageInputs: Record<string, any> = {
  dashboard: {
...
const noviceLexicon: [RegExp, string, string][] = [
...
const translateNeed = (say: string, example?: any) => {
  const hit = example && example.noviceSuggest && example.noviceSuggest.find((n: any) => n.say.toLowerCase() === say.toLowerCase());
...
const toOperator = (screen?: string, trace?: string) => {
...
const spineScreenFor = (intent: string) => ({
...
} as Record<string, string>)[intent] || "spine";
