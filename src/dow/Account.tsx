...
function statusBadge(s: string, tone: string) {
  const map: any = { ok: "green", info: "spruce", warn: "amber" };
...
export function Account({ onNav, onAskAgent, toast }: any) {
  const a = DX.account;
  return (
            ...
            <div className="dx-card">
              ...
              {DX.cases.map(c => (
                <div className="dx-orow" key={c.code} onClick={() => onNav({ name: "case", code: c.code })} style={{ cursor: "pointer" }}>
                  <span className="oi" style={c.env === "plant" ? { background: "var(--k-warm-red-10)", color: "var(--k-warm-red-60)" } : undefined}><Icon name={c.icon} size={16} /></span>
                  <div className="om"><b>{c.title}</b><span>{c.code} · {DX.typeById(c.type)!.name} · {DX.spine[c.stage]}</span></div>
