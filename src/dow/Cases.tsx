...
function Spine({ stage }: any) {
  return (
    <div className="dx-spine">
      {DX.spine.map((s, i) => (
        <React.Fragment key={i}>
          <span className={`dx-spine-node ${i < stage ? "done" : i === stage ? "now" : ""}`}>
            <span className="dot">{i < stage ? <Stroke size={10} sw={3}><polyline points="20 6 9 17 4 12" /></Stroke> : i + 1}</span>
            <span className="lb">{s}</span>
          </span>
          {i < DX.spine.length - 1 && <span className={`dx-spine-bar ${i < stage ? "done" : ""}`} />}
        </React.Fragment>
      ))}
    </div>
  );
...
function DealGates({ c }: any) {
  const deal = DX.dealFor(c.code);
  if (!deal || deal.phase === "support") return null;
  const phaseIdx: any = { inventory: 0, sample: 1, po: 2, reorder: 2 }[deal.phase];
  return (
    <Sec icon="network" title="Deal progression & gates">
      <div className="dx-funnel">
        {DX.dealFunnel.map((p, i) => (
      ...
      <GateList c={c} ids={Object.keys(deal.gates)} title="Gate clearances · agent auto-clears within policy, exceptions route to Dow" />
    </Sec>
  );
}

function GateList({ c, ids, title, onAsk }: any) {
  const deal = (c && DX.dealFor(c.code)) || { gates: {} };
  const rows = ids.filter((id: string) => DX.gates[id]);
  ...
  return (
    <div className="dx-gates">
      ...
      {rows.map((id: string) => {
        const g = DX.gates[id]; const st = (deal.gates[id]) || "progress"; const meta = DX.gateState[st]; const owner = DX.agentById(g.owner)!;
...
function DealTeam({ c }: any) {
  const deal = DX.dealFor(c.code) || { team: [], lead: null };
  const team = (deal.team || []).map((id: string) => DX.agentById(id)).filter(Boolean);
  return (
...
export function CasesHub({ onNav, onAskAgent }: any) {
  return (
            ...
            return (
              <div className="dx-caserow" key={c.code} onClick={() => onNav({ name: "case", code: c.code })}>
                ...
                <div className="cmain">
                  ...
                  <div className="crow">{typeChip(c.type)} <span className="dx-envchip"><Icon name={DX.environments[c.env].icon} size={11} /> {DX.environments[c.env].label}</span></div>
                </div>
                <div className="cmeta">
                  <span className={`b ${c.env === "plant" ? "red" : "spruce"}`} style={c.env === "plant" ? { background: "var(--k-warm-red-10)", color: "var(--k-warm-red-70)", borderColor: "var(--k-warm-red-20)" } : undefined}>{c.status}</span>
                  <span className="cstage">{DX.spine[c.stage]} · stage {c.stage + 1}/{DX.spine.length}</span>
...
export function CaseDetail({ code, onNav, onAskAgent, toast }: any) {
  const c = DX.caseByCode(code) || DX.cases[0];
  const t = DX.typeById(c.type)!;
  const products = (c.products || []).map((id: string) => DX.productById(id)).filter(Boolean);

  function continueAgent(intent?: string) { onAskAgent({ intent: intent || t.mode, text: c.prompt, code: c.code, env: c.env, fresh: true }); }
    ...
    <div className="dx-scroll fade-in">
        ...
        <div className="dx-wrap">
          ...
          <div className="dx-case-top">
            ...
            <div style={{ flex: 1 }}>
              ...
              <h1>{c.title}</h1>
              <div className="crow">{typeChip(c.type)} <span className="dx-envchip"><Icon name={DX.environments[c.env].icon} size={11} /> {DX.environments[c.env].label}</span> <span className={`b ${c.env === "plant" ? "red" : "spruce"}`} style={c.env === "plant" ? { background: "var(--k-warm-red-10)", color: "var(--k-warm-red-70)", borderColor: "var(--k-warm-red-20)" } : undefined}>{c.status}</span></div>
            </div>
      ...
      <div className="dx-wrap">
        <div className="dx-case-grid">
          <div>
            {DX.regAlert.affects.case === c.code && (
              <div className="dx-caseRR">
                <span className="ri"><Icon name="anomaly" size={16} /></span>
                <div className="rb">
                  <div className="rt">RegRadar · proactive regulatory alert <span className="rsev">{DX.regAlert.severity} impact</span></div>
                  <div className="rd"><b>{DX.regAlert.rule}</b> — {DX.regAlert.change} Effective {DX.regAlert.effective}.</div>
                  <div className="ra"><span className="ai-glyph" style={{ width: 13, height: 13 }} /> {DX.regAlert.action}</div>
                </div>
                <div className="rax">
                  ...
                  <button className="dx-btn ghost sm" onClick={() => DX.toOperator(DX.regAlert.spineScreen, c.trace)}><span className="ai-glyph" style={{ width: 12, height: 12 }} /> Open in RegRadar ↗</button>
                </div>
              </div>
            )}
            <Sec icon="chat-bot" title="Application brief">
              <div className="dx-kvgrid">{c.brief.map((b: any, i: number) => <div className="dx-kv" key={i}><span className="k">{b[0]}</span><span className="v">{b[1]}</span></div>)}</div>
            </Sec>
            ...
            {c.experiment && (
              <Sec icon="lightbulb" title="Sample experiment plan" right={<span className="more" onClick={() => onNav({ name: "samplelab", code: c.code })}>Open sample lab</span>}>
                ...
                {c.experiment.rounds ? c.experiment.rounds.map((r: any) => (
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>{c.experiment.packs.map((p: any, i: number) => <span className="dx-pchip" key={i}><Icon name="recommend" size={12} /> {p[0]} · {p[1]}</span>)}</div>
                )}
            ...
            {c.incident && (
              <Sec icon="warning-alt" title="Production incident">
                ...
                <div className="dx-diaggrid">{c.incident.contributors.map((co: any, i: number) => <div className="dx-contrib" key={i}><span className={`bar ${co[2]}`} /><span className="ck">{co[0]}</span><span className="cw">{co[1]}</span></div>)}</div>
                <div className="dx-safety"><div className="sh"><Icon name="error-filled" size={14} /> Safety boundaries — locked <span className="lock">Unalterable</span></div>{c.incident.safety.map((s: string, i: number) => <div className="sr" key={i}><Icon name="checkmark-filled" size={13} /> {s}</div>)}</div>
              </Sec>
            )}
            ...
            {c.guidance && (
              <Sec icon="document-chart" title="Formulation guidance">
                ...
                {c.guidance.ranges.map((r: any, i: number) => <div className="dx-range" key={i}><span className="rk">{r[0]}</span><span className="rv">{r[1]}</span><span className="rn">{r[2]}</span></div>)}
            ...
            {c.compliance && (
              <Sec icon="anomaly" title={`Compliance · ${c.compliance.jurisdiction}`} right={<span className="more" onClick={() => continueAgent("compliance")}>Open in agent</span>}>
                {c.compliance.domains.map((d: any, i: number) => (
            ...
            {c.commercial && (
              <Sec icon="analytics" title="Commercial status" right={<span className="more" onClick={() => continueAgent("conversion")}>Opportunity view</span>}>
                ...
                {c.commercial.blockers.map((b: string, i: number) => <div className="dx-caveat" key={i}><Icon name="warning-alt" size={13} style={{ color: "var(--k-status-warning-110)" }} /> {b}</div>)}
          ...
          <aside>
            <Sec icon="network" title="Case timeline">
              <div className="dx-tl">
                {DX.spine.slice(0, c.stage + 2).map((s, i) => (
              ...
              <button className="dx-btn ghost sm" style={{ width: "100%", marginTop: 10 }} onClick={() => onNav({ name: "history", code: c.code })}><Stroke size={13}><polygon points="6 4 20 12 6 20" fill="currentColor" stroke="none" /></Stroke> Replay full history</button>
            </Sec>
            <DealTeam c={c} />
          </aside>
        </div>
