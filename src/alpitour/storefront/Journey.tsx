...
export function JourneyGraph({ mode, versions, holdRef, turiPrice, turiRepriced, mdGenerated, mdSent }: {
  mode: string; versions?: any[]; holdRef?: string;
  turiPrice?: number; turiRepriced?: boolean; mdGenerated?: boolean; mdSent?: boolean;
}) {
  const d = (tredi as any).journey[mode];
  ...
  if (mode === 'turisanda' && turiRepriced) {
    prices = d.prices.map((v: number, i: number) => (i === d.repriceIdx ? turiPrice : v));
  }
    ...
    const datasets: any[] = [
      {
        ...
        pointRadius: prices.map((_: any, i: number) => (mode === 'turisanda' && turiRepriced && i === d.repriceIdx ? 5 : 0)),
    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { backgroundColor: '#0F172A', padding: 10, cornerRadius: 4, callbacks: { label: (c: any) => c.dataset.label.split(' · ')[0] + ': ' + fmtEUR(c.raw) } },
