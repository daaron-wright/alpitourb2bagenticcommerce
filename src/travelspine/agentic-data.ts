/* ============================================================
   Agentic capabilities — the Travel Assistant agent roster + the 11
   workflows that support the B2B concierge experience.
   Each workflow carries an ordered agent chain; each step has
   an action, a tool, and an optional PAC gate so TravelRadar
   and the floating pipeline can trace it step-by-step.
   ============================================================ */
...
const A: Record<string, any> = {
  ...
  inventory:     { id: "inventory",     name: "Inventory Agent",       short: "Inventory",    icon: "network",          c: "#1E4E79", tint: "#E2EAF2", task: "execution",      owns: "Packages, hotels, Airline Partner flights and allotments — availability, yield status, sold-out detection." },
...
export const agenticAgents: Record<string, any> = A;

export const agenticAgentList: any[] = [
  "intent", "requirements", "context", "inventory", "recommendation", "pricing",
  "itinerary", "proposal", "booking", "upsell", "modification", "radar",
].map(id => A[id]);

/* ---------- The 11 workflows ---------- */
export const agenticWorkflows: any[] = [
  {
  ...
  },
  {
    ...
    steps: [
      { agent: "inventory", action: "Search packages, hotels and Airline Partner flights against the structured requirements.", tool: "inventory.search" },
  ...
  },
  {
    ...
    steps: [
      ...
      { agent: "pricing", action: "Re-check margin and refund exposure on the re-accommodation.", tool: "yield.recompute",
        pac: { rule: "PROM-covenant-agency", q: "Does the re-point preserve room class + family amenities?", outcome: "route", note: "Covenant-affecting — confirmed by the planner who owns the desk." } },
...
export const agenticWorkflowById: Record<string, any> = {};
agenticWorkflows.forEach(w => { agenticWorkflowById[w.id] = w; });
