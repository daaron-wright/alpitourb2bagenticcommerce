/* ============================================================
   Travel Group Agentic Intelligence Platform — compiled state machine
   Source of truth: travel_agentic_intelligence_platform_state_machine.json
   (journey_id: travel_agentic_intelligence_platform_v1 · 37 states)
   ============================================================ */

// Module → palette (Shidoka categorical, order-locked by prominence)
const MOD: Record<string, { c: string; label: string }> = {
  "Platform":                   { c: "#334155", label: "Platform" },
  "Travel Assistant":                    { c: "#475569", label: "Travel Assistant" },
...
const S: any[] = [
  ...
  { n: "Customer Intent Branch", s: "Intent branch", m: "Travel Assistant", t: "decision", ctrl: "customer_workflow",
  ...
  { n: "Package Recommendation Analysis", s: "Package rec", m: "Travel Assistant", t: "process", ctrl: "package_recommendation_status",
    tx: [["TravelRadar Compliance Check", "ranked"], ["Human Escalation Router", "no candidate"], ["Human Escalation Router", "low conf"]] },
  { n: "Itinerary Guidance Analysis", s: "Itinerary", m: "Travel Assistant", t: "process", ctrl: "itinerary_guidance_status",
  ...
  { n: "Proactive Rebooking Analysis", s: "Rebooking", m: "Travel Assistant", t: "process", ctrl: "rebooking_analysis_status",
  ...
  { n: "Cited Response Generation", s: "Cited response", m: "Travel Assistant", t: "process", ctrl: "response_status",
    tx: [["Next Action Routing", "ready"], ["Human Escalation Router", "needs review"], ["Human Escalation Router", "citation gap"]] },
  { n: "Next Action Routing", s: "Next action", m: "Travel Assistant", t: "decision", ctrl: "next_action",
...
function node(s: any) { return { id: s.n, label: s.s, cat: s.m, type: s.t }; }
function usedCats(states: any[]) {
  const c: Record<string, any> = {};
  states.forEach(s => { if (MOD[s.m]) c[s.m] = MOD[s.m]; });
  return c;
}
function buildSub(names: string[]) {
  const set = new Set(names);
  const states = S.filter(s => set.has(s.n));
  const links: any[] = [];
  states.forEach(s => s.tx.forEach(([tg, val]: [string, string]) => { if (set.has(tg)) links.push({ source: s.n, target: tg, rel: val }); }));
  return { cats: usedCats(states), nodes: states.map(node), links };
}
function buildFull() {
  const links: any[] = [];
  S.forEach(s => s.tx.forEach(([tg, val]: [string, string]) => { if (byName[tg]) links.push({ source: s.n, target: tg, rel: val }); }));
...
const LIVE: any[] = [
  ...
  { n: "Downstream Notification", decision: "sent", note: "The trade desk and the agency are notified; the Traveler Family A's 14-Aug date is protected." },
...
].map((x: any) => ({ ...x, m: byName[x.n].m, ctrl: byName[x.n].ctrl, type: byName[x.n].t }));
