/* ============================================================
   Travel Group Platform · traveler view (Consumer Travel Portal)
   Migrated from travel/platform/traveler.jsx
   ============================================================ */
...
function THeader({ onAssistant }: { onAssistant: () => void }) {
  return (
...
function TTrip({ b, onAssistant }: { b: any; onAssistant: () => void }) {
  const steps = [
...
function TSheet({ open, onClose, b, inline, boot }: any) {
  ...
  function photoDone(skipped: boolean) {
    ...
    bus.push('feed', { who: 'traveler', t: 'Sent a photo with the request — Travel Assistant is reading the preferences' });
    later(1600, () => {
  ...
  function confirmNeeds() {
    ...
    setTimeout(() => { bus.push('feed', { who: 'traveler', t: 'Travel Assistant agentic search · 142 stays → 12 match → 3 fit the family profile' }); }, 3200);
    setTimeout(() => {
  ...
  function sendToAgency() {
    ...
    later(1400, () => {
      bus.emit('handoff');
      bus.push('feed', { who: 'traveler', t: 'Request sent to Partner Travel Agency — with a 24h price lock' });
      push({ kind: 'handoff' });
