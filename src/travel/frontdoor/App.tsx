/* ============================================================
   Travel Assistant — Travel Group's travel concierge (clean conversational UI)
   Migrated from travel/frontdoor3/app.jsx
   ============================================================ */
import React, { useState, useRef, useEffect } from 'react';
import { SCENARIOS } from './data';
import { AiIcon } from '@/shared/AiIcon';

const Ki = ({ name, size = 16 }: { name: string; size?: number }) =>
  name === 'ai' || name === 'ai-insight'
    ? <AiIcon size={size} insight={name === 'ai-insight'} />
    : <svg className="ki" style={{ width: size, height: size }}><use href={`#icon-${name}`} /></svg>;

const SUGG = [
  { id: 'cotswolds', t: 'Somewhere warm in October — relaxed, good food, short flight' },
...
const STATUS_LINES: Record<string, string[]> = {
  cotswolds: ['Reading your request…', 'Searching Travel Group, Family Travel and Premium Travel…', 'Putting your trip together…', 'Final checks…'],
...
function detectScenario(q: string) {
...
export function App() {
  const [view, setView] = useState<'chat' | 'agencies'>('chat');
  ...
  const timers = useRef<any[]>([]); const toastT = useRef<any>(null);
  ...
  function showToast(t: string) { setToast(t); clearTimeout(toastT.current); toastT.current = setTimeout(() => setToast(null), 2600); }

  function ask(text: string, forcedId?: string) {
  ...
  function understandingHtml(sc: any) {
  ...
  function followUp(kind: string, sc: any) {
  ...
  const inputEl = (dock: boolean) => (
    <div className={dock ? 'g-dock' : 'g-inputwrap'}>
      <div className={dock ? 'g-dock-in' : ''}>
        ...
        {dock && <div className="g-dock-note">Travel Assistant answers from Travel Group's own catalogues — Travel Group, Family Travel and Premium Travel.</div>}
      </div>
    </div>
  );
    ...
    <div className="g-app">
      <header className="g-head">
        <div className="g-head-in">
          <div className="g-logo" onClick={() => location.reload()}>travel<span> · Travel Assistant</span></div>
          <nav className="g-nav">
      ...
      {view === 'chat' && !started && (
        <div className="g-center">
          <div className="g-center-in">
            ...
            <p className="g-foot-note">Answers grounded in Travel Group's own catalogues · hand to a human anytime</p>
          </div>
        </div>
      )}
            ...
            if (it.kind === 'ai') return (
              <div className="g-msg" key={i}>
                ...
                <div className="body">
                  ...
                  {it.brief && <div className="g-brief">{it.brief.map((b: string, j: number) => <span key={b} className={`g-bchip ${j >= it.brief.length - 3 ? 'warm' : ''}`}>{b}</span>)}</div>}
                </div>
              </div>
            );
            if (it.kind === 'result') {
                  ...
                  <div className="body">
                    <div className="g-card">
                      ...
                      <div className="g-card-b">
                        ...
                        <div className="g-why">
                          {r.why.map(([a, b]: [string, string]) => <div className="w" key={a}><Ki name="checkmark-filled" size={13} /><span><b>{a}</b> · {b}</span></div>)}
                        </div>
                        <div className="g-segs">
                          {r.segs.map(([ic, t]: [string, string]) => <span className="g-seg" key={t}><Ki name={ic} size={12} /> {t}</span>)}
                        </div>
      ...
      {view === 'chat' && started && inputEl(true)}
        ...
        <div className="g-scrim" onClick={(e) => { if (e.target === e.currentTarget) setAgency(null); }}>
          <div className="g-modal">
            ...
            <div className="box">
              ...
              <div className="r"><span>Your agency</span><span>Partner Travel Agency · London</span></div>
            </div>
...
function AgencyFlow({ showToast }: { showToast: (t: string) => void }) {
  const [open, setOpen] = useState<string | null>(null);
  const [step, setStep] = useState('context');
  const [adds, setAdds] = useState(['transfer']);

  const WL = [
    { id: 'travelerA', t: 'Traveler Family A · Sharm el-Sheikh', m: '2 adults + 1 child (2y) · 12–19 Aug · ≤ £3,500 · all inclusive', src: 'From Consumer Travel Portal — full trip context attached', badge: 'handoff', bl: 'Handoff' },
    { id: 'travelerF', t: 'Traveler F · Greek islands', m: '1 adult · September · boutique', src: 'Email · this morning', badge: 'new', bl: 'New' },
    { id: 'travelerG', t: 'Traveler Group G · ski week', m: '12 pax · hotel price changed', src: 'Price alert · needs review', badge: 'update', bl: 'Update' },
  ];
  const OPTS = [
  ...
  function pick(id: string) {
    if (id !== 'travelerA') { showToast('Open Traveler Family A to follow the guided flow.'); return; }
    setOpen(id); setStep('context');
  }

  const Conn = ({ items }: { items: string[] }) => (
  ...
  return (
    <div className="a-wrap">
      <div className="a-intro">
        ...
        <p className="sub">Requests arrive with the customer's full context — whether they spoke to Travel Assistant on Consumer Travel Portal, emailed, or called. You pick up where they stopped: search, price, propose, hold and change, all in one place.</p>
      </div>
        ...
        <div className="a-work">
          {!open && (
            <div className="a-card" style={{ textAlign: 'center', padding: '44px 24px', color: 'var(--fg-muted)', fontSize: 13.5 }}>
              Open a request to start — <b style={{ color: 'var(--fg-1)' }}>Traveler Family A</b> just arrived from Consumer Travel Portal with their trip context attached.
            </div>
          )}
            ...
            <>
              ...
              <div className="a-card">
                <dl className="a-kv">
                  ...
                  <dt>Board</dt><dd>All inclusive · from London</dd>
              ...
              <Conn items={['Live inventory', 'Airline Partner allotments', 'Customer context']} />
            </>
          )}
          ...
          {open && step === 'proposal' && (
            <>
              ...
              <div className="a-card">
                <div className="a-seg"><Ki name="arrow-up-right" size={15} /> <span>Flight · Airline Partner — London → Sharm, direct</span><span className="p" style={{ fontWeight: 400, color: 'var(--fg-muted)' }}>included</span></div>
          ...
          {open && step === 'done' && (
            <>
              ...
              <div className="a-card">
                ...
                <p style={{ fontSize: 12.5, color: 'var(--fg-2)', marginTop: 10, lineHeight: 1.5 }}>Proposal sent to Traveler Family A. If they change their mind — dates, hotel, services — you change it here: re-priced instantly, every version kept, <b>no support ticket</b>.</p>
              </div>
