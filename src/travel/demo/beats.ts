/* ============================================================
   Travel Assistant 2.0 — Guided demo · the 12 story beats
   Migrated from travel/demo/beats.js
   ============================================================ */

import type { DemoBeat } from '@/shared/types';

// tiny pub/sub the storefront app broadcasts its stage into
let demoBusState: { stage: string; approval: boolean; [key: string]: any } = {
  stage: 'browse',
  approval: false,
};
let demoBusSubs: Array<(state: typeof demoBusState) => void> = [];

export const DemoBus = {
  s: demoBusState,
  subs: demoBusSubs,
  set(p: Partial<typeof demoBusState>): void {
    Object.assign(this.s, p);
    this.subs.forEach((f) => { try { f(this.s); } catch (e) {} });
  },
  on(f: (state: typeof demoBusState) => void): () => void {
    this.subs.push(f);
    return () => { this.subs = this.subs.filter((x) => x !== f); };
  },
};

const AG = 'Travel Assistant', LY = 'LyteAI', EB = 'Easybooking';
...
export const DEMO_BEATS: DemoBeat[] = [
  /* 1 */ {
    ...
    show: ['Agency storefront — Partner Travel Agency · Gold Partner', 'Two live requests on the desk', 'Travel Orchestrator panel embedded in the flow, not bolted on'],
  ...
  /* 2 */ {
    ...
    show: ['Free-text Family A brief arrives from Consumer Travel Portal', 'Travel Assistant extracts 6 structured requirement chips', 'Detected signals: baby pool · kids\' club'],
...
#  -> ["Premium Travel", "Travel Group"]   weighted first in ranking
...
candidate {
  ...
  input.cand.brand in data.travel.world.brands
}

# Re-rank toward the signals Travel Assistant detected at capture.
...
flag[msg] {
  ...
  /* 8 */ {
    ...
    point: 'The agent stays in one flow while the system orchestrates the right capability behind the scenes: Travel Assistant interprets, LyteAI composes the itinerary, Easybooking structures the proposal and booking.',
    show: ['Package assembled: Airline Partner flight · hotel · board · transfer · add-ons', 'Itinerary composed (LyteAI)', 'Proposal + booking path structured (Easybooking)'],
...
valid {
  ...
  /* 10 */ {
    ...
    say: 'Travel Assistant can prepare and guide, but the agent stays in control when the action has commercial consequence.',
    cap: [EB], stage: 'held',
