/* ============================================================
   Travel Assistant B2B Concierge — app shell + scripted state machine
   Migrated from travel/concierge/app.jsx
   ============================================================ */
import React, { useState, useRef } from 'react';
import { fmtGBP, KAF, hotels, addOns as allAddOns } from './data';
import {
  Ki, WelcomeCard, RequestCapturedCard, TripContextCard, TripStrip,
...
import { ChatPanel, AgentTrace } from './Chat';
...
export function App() {
  ...
  const [msgs, setMsgs] = useState<any[]>([{ role: 'agent', html: 'Welcome back. I can help you <b>build</b>, <b>check</b>, <b>modify</b>, or <b>hold</b> a travel proposal. What is the customer looking for?' }]);
  const [chips, setChips] = useState<any[]>([{ id: 'start', label: 'Start new customer request', kind: 'primary' }]);
  const [trace, setTrace] = useState<any[]>([]);
  const [detailSel, setDetailSel] = useState<string[]>([]);
  ...
  const hotel = hotelId ? hotels[hotelId] : hotels.coral;

  function pushUser(text: string) { setMsgs((m) => [...m, { role: 'user', html: text }]); }
  function pushAgent(html: string) { setMsgs((m) => [...m, { role: 'agent', html }]); }
  function addTrace(items: any[]) {
  ...
  function orchestrate({ user, traceItems, reply, nextWs, nextStage, nextChips, delay = 1050, after }: any) {
  ...
  function totals(base: number, adds: string[]) {
    const addSum = allAddOns.filter((a) => adds.includes(a.id)).reduce((s, a) => s + a.price, 0);
  ...
  function actStart(userText?: string) {
    orchestrate({
      user: userText || 'I have a family of 3 looking for an all-inclusive Egypt package from London in August, budget around £3,500. They were hoping for Jaz Mirabel Beach.',
      traceItems: [
        { agent: 'KAF · WebsiteAPIService', icon: 'network', message: 'Work package wpkg_travel_001 created · session authenticated (EasyBook)', ...KAF.discover },
  ...
  function actConfirmDetails() {
    orchestrate({
      ...
      reply: "Thanks. I'll search for a <b>7-night all-inclusive family package</b> from London to Egypt, with flexibility for similar family-friendly resorts if Jaz Mirabel Beach is unavailable. First, let me apply your agency context.",
  ...
  function actApplyContext() {
    orchestrate({
      user: "Apply Partner Travel Agency's agency context.",
      ...
      reply: "I've applied <b>Partner Travel Agency's Gold Partner context</b> and standard commission rules — pricing and recommendations will reflect your tier automatically.",
  ...
  function actRetrieval(userText?: string) {
    orchestrate({
  ...
  function actAvailability(userText?: string) {
    orchestrate({
  ...
  function actAlternatives(userText?: string) {
    orchestrate({
  ...
  function actSelectHotel(id: string) {
    setHotelId(id);
    const h = hotels[id];
    orchestrate({
      user: `Select ${h.name}.`,
      traceItems: [
        ...
        { agent: 'Inventory Agent', icon: 'analytics', message: 'Flight (Airline Partner, LHR→SSH) and hotel availability confirmed', ...KAF.plan },
      ],
      reply: `I've assembled a 7-night package with <b>flights from London</b>, <b>${h.name}</b>, all-inclusive board, and standard airport transfer. You can adjust any segment, or review suggested add-ons.`,
  ...
  function actGenerateProposal() {
    ...
    orchestrate({
      ...
      traceItems: [
        { agent: 'Pricing Agent', icon: 'analytics', message: `Gold-tier pricing + 12% commission applied (${fmtGBP(t.commission)}) · pricing-mcp`, ...KAF.validate },
      ...
      reply: `Your <b>agency-ready proposal</b> is ready — total client price <b>${fmtGBP(t.total)}</b>, your commission <b>${fmtGBP(t.commission)}</b>. Pricing and commission rules have been applied and validated.`,
  ...
  function actChangeDates(userText?: string) {
    orchestrate({
      ...
      reply: `Good news — the same hotel and flight route are <b>available one week earlier</b> (5–12 August). The updated package price is <b>${fmtGBP(v2Total)}</b>, which is <b>£100 lower</b> than the original proposal.`,
  ...
  function actWaterpark(userText?: string) {
    orchestrate({
  ...
  function actApplySwitch() {
    ...
    orchestrate({
      ...
      reply: `Done — the proposal now uses <b>Sinai Blue Lagoon Resort</b> at <b>${fmtGBP(3795)}</b>. Here's everything you can do next.`,
  ...
  function onPackageEdit(k: string) {
  ...
  function onHoldAction(k: string) {
  ...
  function onNba(k: string, label: string) {
  ...
  if (ws === 'addons') view = (
    <>
      ...
      <AddOnsPanel selected={addOns} onToggle={(id: string) => setAddOns((s) => (s.includes(id) ? s.filter((x) => x !== id) : [...s, id]))} onGenerate={actGenerateProposal} busy={busy} />
    </>
  );
  ...
  return (
    <div className="cg-app">
      <header className="cg-head">
        ...
        <div className="cg-agency"><span>Partner Travel Agency</span><span className="gold">Gold Partner</span></div>
