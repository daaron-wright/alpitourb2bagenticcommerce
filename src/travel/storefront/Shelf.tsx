...
import { fmtGBP, brands, products, SHELVES, BRIEF, chips, desk, orchestrator } from './data';
...
export function ChipsBar({ blockers, onFix, stage }: { blockers: any[]; onFix: (b: any) => void; stage: string }) {
  const rq = ((orchestrator as any).requests || []).find((r: any) => r.id === 'travelerA') || {};
  ...
  return (
    <div className={`sf-cust ${expanded ? 'open' : ''}`} data-screen-label="Customer brief">
      <button className="sf-cust-bar" onClick={() => setOpen((o) => !o)} aria-expanded={expanded}>
        ...
        <span className="src"><Inf size={11} /> via Travel Concierge · Consumer Travel Portal</span>
...
function JourneyTakeoff({ awaiting, stage }: { awaiting: boolean; stage: string }) {
...
function DeskOverview({ onToast }: { onToast: (msg: string) => void }) {
  ...
  const dests = [
    { id: 'travelerA', ll: [51.507, -0.128] as [number, number], who: 'Family A', place: 'London · United Kingdom', tone: '#0F62FE' },
    { id: 'travelerD', ll: [3.2, 73.0] as [number, number], who: 'Traveler D', place: 'Maldives', tone: '#7A2048' },
    { id: 'travelerE', ll: [28.29, -16.62] as [number, number], who: 'Traveler E', place: 'Tenerife', tone: '#1F8A5B' },
    { id: 'travelerG', ll: [57.12, -4.71] as [number, number], who: 'Traveler G', place: 'Scottish Highlands', tone: '#29707A' },
    { id: 'travelerF', ll: [36.39, 25.46] as [number, number], who: 'Traveler F', place: 'Santorini', tone: '#8a6116' },
    { id: 'travelerB', ll: [35.68, 139.77] as [number, number], who: 'Couple B', place: 'Japan', tone: '#FF462D' },
    { id: 'travelerC', ll: [50.72, -3.53] as [number, number], who: 'Traveler C', place: 'Devon', tone: '#0F62FE' },
  ];
  ...
  const evd: Record<number, { c: string; t: string }> = {
    2: { c: '#1F8A5B', t: 'Traveler C · Devon departs' },
    7: { c: '#E6A100', t: 'Traveler D · Maldives hold ends' },
    8: { c: '#0F62FE', t: 'Traveler E · Tenerife hold ends' },
    12: { c: '#FF462D', t: 'Family A · United Kingdom departs' },
  };
...
export function DeskBoard({ awaiting, busState, onAccept, onToast, stage, onResume, showFamilyA }: {
  awaiting: boolean; busState: any; onAccept: () => void;
  onToast: (msg: string) => void; stage: string; onResume: () => void; showFamilyA?: boolean;
}) {
  ...
  const familyA: any = { id: 'travelerA', who: 'Traveler Family A', cat: 'United Kingdom · Family Travel', trip: cs.trip, dates: '12–22 Aug', val: cs.val, src: cs.src, srcIcon: 'chat-bot', tag: cs.tag, tagk: cs.tagk, primary: cs.col !== 'booked', cday: 12, cnote: 'departs', act: cs.act };
  const inject = (key: string, arr: any[]) => (showFamilyA !== false && key === cs.col) ? [familyA, ...arr] : arr;
  ...
  return (
    <section className="sf-board" data-screen-label="Agent desk">
                  ...
                  {col.clients.map((c: any) => {
                    ...
                    return (
                      <article className={`bd-card ${c.primary ? 'primary' : ''} ${c.urgent ? 'urgent' : ''}`} key={c.id}
                        onClick={click} data-screen-label={isFamilyA ? 'Incoming handoff' : undefined}>
      ...
      {view === 'calendar' && (() => {
        ...
        const shortName = (n: string) => n.replace(/^(The |Family|Group|Couple|Sig\.ra|Sig\.) /, '');
        return (
