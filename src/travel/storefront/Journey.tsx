...
import { fmtGBP, orchestrator } from './data';
import { Ki } from './Primitives';

Chart.register(...registerables);

const FLAG_COL: Record<string, string> = { keep: '#16A34A', consider: '#F59E0B', address: '#DC2626' };
const FLAG_LBL: Record<string, string> = { keep: 'keep as is', consider: 'consider', address: 'address' };

/* custom plugin — stay bands, flight marks, event lines, flag badges */
const journeyDeco = {
  id: 'journeyDeco',
  afterDatasetsDraw(chart: any, args: any, opts: any) {
    const { ctx, chartArea: a, scales: { x } } = chart;
    if (!a) return;
    ctx.save();
    (opts.bands || []).forEach((b: any) => {
    ...
    (opts.flags || []).forEach((f: any, i: number) => {
    ...
    (opts.events || []).forEach((e: any, i: number) => {
...
export function JourneyGraph({ mode, versions, holdRef, signaturePrice, signatureRepriced, mdGenerated, mdSent }: {
  mode: string; versions?: any[]; holdRef?: string;
  signaturePrice?: number; signatureRepriced?: boolean; mdGenerated?: boolean; mdSent?: boolean;
}) {
  const d = (orchestrator as any).journey[mode];
  const canvasRef = React.useRef<HTMLCanvasElement>(null);
  const chartRef = React.useRef<any>(null);

  /* ---- derive bands + events from portal state ---- */
  let bands: any[] = [], events: any[] = [];
  if (mode === 'family') {
    ...
    if (mdSent) events.push({ x: 7, label: 'proposal.md sent to the Family As', icon: 'arrow-up-right' });
  } else {
    bands = d.stops.map((s: any) => ({ from: s.from, to: s.to, fill: s.color + '1A', edge: s.color, text: s.color, label: s.label }));
    bands[0].flights = true; bands[bands.length - 1].flights = true;
    if (signatureRepriced) events.push({ x: d.repriceIdx, label: 'Live price watch · Airline Partner −£120, repriced', icon: 'analytics' });
    if (mdGenerated) events.push({ x: d.repriceIdx, label: 'proposal-travelerB.md written', icon: 'document-chart' });
    if (mdSent) events.push({ x: d.repriceIdx, label: 'Sent to the Couple Bs', icon: 'arrow-up-right' });
  }

  /* ---- price series (signature reprices live) ---- */
  let prices = d.prices;
  if (mode === 'signature' && signatureRepriced) {
    prices = d.prices.map((v: number, i: number) => (i === d.repriceIdx ? signaturePrice : v));
  }
    ...
    const datasets: any[] = [
      {
        ...
        pointRadius: prices.map((_: any, i: number) => (mode === 'signature' && signatureRepriced && i === d.repriceIdx ? 5 : 0)),
    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { backgroundColor: '#0F172A', padding: 10, cornerRadius: 4, callbacks: { label: (c: any) => c.dataset.label.split(' · ')[0] + ': ' + fmtGBP(c.raw) } },
          journeyDeco: { bands, events, flags: d.flags },
        } as any,
        scales: {
          x: { grid: { display: false }, ticks: { color: '#64748B', maxTicksLimit: 8, font: { size: 10 } } },
          y: { min: d.yMin, max: d.yMax, grid: { color: '#F1F5F9' }, ticks: { color: '#64748B', font: { size: 10 }, callback: (v: any) => '£' + (v >= 1000 ? (v / 1000).toFixed(1).replace('.0', '') + 'k' : v) } },
        },
      },
      plugins: [journeyDeco as any],
    });
