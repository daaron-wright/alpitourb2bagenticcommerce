...
import { fmtGBP, products, availability, addons, brands, priceCalendar } from './data';
        ...
        <div className="sf-drawer-body">
          ...
          <div className="sf-sec">
            ...
            <div className="sf-rate">
              ...
              {(addons as any[]).map((a: any) => selAddons.includes(a.id) && <div className="row" key={a.id}><span className="l">{a.name}</span><b className="num">{fmtGBP(a.price)}</b></div>)}
  ...
  return (
    <section className="sf-pkg" data-screen-label="Proposal panel">
      ...
      <div className="sf-lines">
        ...
        {adds.map((a: any) => <div className="sf-line" key={a.id}><Ki name="recommend" size={14} /> <span>{a.name} <span className="m">{a.why}</span></span><span className="pr num">{fmtGBP(a.price)}</span></div>)}
      ...
      {v.delta && (
        <div className="sf-diff" style={{ marginTop: 11 }}>
          <b>{v.versionId} vs {v.delta.vs}:</b> {v.delta.amount < 0 ? '−' : '+'}{fmtGBP(Math.abs(v.delta.amount))} — {v.delta.reason}. Hotel, room type and amenities are identical. <SrcChip ids={['pol1']} onSource={onSource} />
        </div>
      )}
    </section>
  );
