/* ============================================================
   ...
   Migrated from travel/storefront/app.jsx
...
import {
  ...
  MdPreview, SignatureTravelPanel, JourneyBoard,
} from './Orchestrator';
...
export const App: React.FC = () => {
  ...
  function startLive(id: string) {
    ...
    const evs = (orchestrator.liveEvents as any)[id] || orchestrator.liveEvents.family;
    const fire = () => {
      ...
      if (ev.priceDelta) { setSignaturePrice((p: number) => p + ev.priceDelta); setSignatureRepriced(true); showDelta(ev.delta); }
  ...
  function workRequest(id: string) {
    ...
    const rq = orchestrator.requests.find((r: any) => r.id === id);
    const cl = orchestrator.clusters.find((c: any) => c.id === rq.cluster);
    setRequest(id);
    setBrand(cl.brand);
    startLive(cl.brand);
  ...
  function sendMd() {
    ...
    if (!signatureMode) { bus && bus.emit('proposal', { file: 'proposal-familyA.md' }); pbFeed('Proposal sent — it\'s on your trip page'); }
    pushHandoff({
  ...
  function approveV3() {
    ...
    orchestrate({
      ...
      then: () => {
        ...
        bus && bus.emit('v2', { total: 7185, window: '15–25 August · Cornwall by ferry' });
    ...
    <div className={`sf-app ${t.density === 'dense' ? 'dense' : ''} ${t.customerSafe ? 'safe' : ''} ${t.showPhotos ? '' : 'noimg'}`}>
      ...
      <div className={`sf-main ${railOpen ? '' : 'rail-closed'}`}>
        <div className="sf-shelf-col" ref={shelfRef} data-screen-label={selling ? (stage === 'signature' ? 'Signature Travel curated' : 'Selling shelf') : 'Storefront'}>
          {selling && !homeView && stage !== 'signature' && <ChipsBar blockers={blk} onFix={fixBlocker} stage={stage} />}
          <div className="sf-shelf-in">
            {(!selling || homeView) && (
              <DeskBoard awaiting={awaitingInquiry} busState={busState} onAccept={acceptHandoff} onToast={showToast} stage={stage} onResume={() => { setHomeView(false); scrollTop(); }} showFamilyA={inquiryShown || stage !== 'browse'} />
            )}
            {!homeView && stage === 'signature' && (
              <>
                <SignatureTravelPanel price={signaturePrice} repriced={signatureRepriced} onGenMd={() => onQuick({ id: 'genmd' })} onSource={onSource} />
                <JourneyGraph mode="signature" signaturePrice={signaturePrice} signatureRepriced={signatureRepriced} mdGenerated={mdGen} mdSent={mdSent} />
                <MapPanel mode="signature" />
              </>
            )}
            ...
            {!homeView && showRanked && (
              <>
                <SoldOutBanner />
                <MapPanel mode="family" onOpenPin={(id: string) => { if ((products as any)[id]) setDrawerId(id); }} />
      ...
      {mdOpen && <MdPreview brand={brand || 'family'} price={brand === 'signature' ? signaturePrice : ((versions.find((v: any) => v.status === 'held') || versions[0] || {}).total || 3465)} holdRef={holdRef || undefined} onClose={() => setMdOpen(false)} onSend={() => onQuick({ id: 'sendmd' })} />}
