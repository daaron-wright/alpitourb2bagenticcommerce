/* ============================================================
   EasyBook Next — B2B Storefront fixtures
   Agency-grade catalogue: net rates, commission, live allotment,
   provenance receipts. Deterministic demo around WP-2231 (Family A).
   Migrated from travel/storefront/data.js
   ============================================================ */

export const fmtGBP = (n: number): string => '£' + n.toLocaleString('en-GB');
...
/* ---- provenance receipts ---- */
export const receipts: Record<string, any> = {
...
export const BRIEF = 'Traveler Family A — 2 adults + 1 child (2 years) from Toronto — 10 days around the United Kingdom in August: London, Bath and Cornwall. They love food, history and slow mornings. Budget ≈ CA$11,000 (≤ £7,400 all-in). Non-negotiables: family rooms, air-con and short transfers.';
...
export const availability: Record<string, any[]> = {
  coral: [
...
export const priceCalendar: Record<string, any> = {
  ...
  items: [
    {
      ...
      meta: 'booking.hold.extend · write · needs you', approveLabel: 'Approve & send',
    ...
    },
    {
      ...
      meta: 'pricing.reprice · read-only · done overnight', approveLabel: 'Share new price',
    ...
    },
    {
      ...
      meta: 'crm.followup.draft · read-only · done overnight', approveLabel: 'Send follow-up',
      delta: 'Traveler F follow-up sent · reply lands in your worklist',
    },
  ],
};

export const orchestrator: any = {
  stickies: {
  ...
  journey: {
    family: {
      ...
      flags: [
        ...
        { from: 4, to: 4, kind: 'consider', label: 'Mid-August holiday peak · 15 Aug', detail: 'National holiday demand — allotments and trains fill fast around the 15th, so confirm holds early.' },
        { from: 12, to: 12, kind: 'consider', label: 'Cornwall road risk', detail: 'A coastal-road closure can disrupt transfers; ferry re-routes are the fallback used by the V3 recovery.' },
        { from: 4, to: 14, kind: 'keep', label: 'Family rooms & short transfers praised', detail: 'Review sentiment is 4.6/5 on the stated must-haves — keep them prominent in the proposal.' },
        { from: 11, to: 14, kind: 'consider', label: 'Busy afternoon feedback', detail: 'Reviews mention crowded city afternoons — favour slow mornings when travelling with a 2-year-old.' },
      ],
    },
    signature: {
      ...
      flags: [
        { from: 0, to: 3, kind: 'consider', label: 'Typhoon tail risk · early Oct', detail: 'Pacific storm tails can affect Tokyo arrivals — keep rebooking flexibility on the outbound.' },
        { from: 9, to: 12, kind: 'keep', label: 'Autumn foliage begins in Kyoto', detail: "Foliage season opens — the D6–8 Kyoto window lands exactly on it. Don't move it." },
        { from: 12, to: 14, kind: 'address', label: 'JAL return fares climbing', detail: 'Returns after 22 Oct are rising — lock the KIX→LHR leg this week.' },
      ],
    },
  },
  japan: {
...
// Backward-compat export for components that destructure window.SF style
export const SF = {
  fmtGBP, brands, receipts, products, SHELVES, BRIEF, chips, blockers, addons,
  availability, PLAN, STAGE_LABEL, desk, pulse, priceCalendar, fitMatrix,
  milestones, overnight, orchestrator,
};
