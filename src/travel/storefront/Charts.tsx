...
import { fmtGBP, pulse, priceCalendar, fitMatrix, milestones, products, desk } from './data';
      ...
      build={() => ({
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: { callbacks: { label: (x: any) => x.datasetIndex === 0 ? 'Budget cap £3,500' : ' £' + x.raw.toLocaleString('en-GB') + (x.dataIndex === pc.requested ? ' · requested window' : '') } },
          },
          scales: {
    ...
    <ChartCard
      ...
      build={() => ({
        ...
        options: {
          ...
          plugins: {
            legend: { display: false },
            tooltip: { callbacks: { label: (x: any) => x.dataIndex === 1 ? ` ${up ? '+' : '−'}£${Math.abs(delta)}` : ' £' + x.raw[1].toLocaleString('en-GB') } },
          },
          scales: {
            x: { ...grid, grid: { display: false } },
            y: { ...grid, min: lo, max: hi, ticks: { callback: (v: number) => '£' + v.toLocaleString('en-GB') } },
          },
        },
      })}
    />
    ...
    <ChartCard
      ...
      build={() => ({
        ...
        data: {
          labels: ms.labels,
          datasets: [
            { label: 'With Travel Orchestrator', data: ms.assistant, backgroundColor: '#29707A', borderRadius: 2, barThickness: 13 },
