/* ============================================================
   ...
   Migrated from travel/easybook/components.jsx
   ============================================================ */
...
export function Card({ icon, title, sub, badges, children }: { icon?: string; title?: string; sub?: string; badges?: React.ReactNode; children?: React.ReactNode }) {
  return (
...
export const Fresh = ({ kind, children }: { kind: string; children: React.ReactNode }) => (
  <span className={`eb-fresh ${kind}`}>
    <Ki name={kind === 'live' ? 'checkmark-filled' : kind === 'stale' ? 'warning-alt' : 'information'} size={10} /> {children}
  </span>
);

export function Receipts({ ids }: { ids: string[] }) {
  return (
...
export function SummaryRail({ stageIdx, needs, blockers, signals, selected, onFixBlocker }: any) {
    ...
    <aside className="eb-rail-l">
      ...
      {blockers.length > 0 && (
        <div className="eb-sec">
          ...
          <div className="eb-empty-note">Tap a question to resolve it — Travel Assistant suggests the customer's answer from the brief.</div>
        </div>
      )}
      <div className="eb-sec">
      ...
      <div className="eb-sec">
        ...
        {sel
          ? <div className="eb-selpkg"><div className="t">{sel.hotelName}</div><div className="s">12–19 Aug · 7 nights · all inclusive · Airline Partner LHR→SSH</div><div className="p">{fmtGBP(sel.price)}</div></div>
  ...
  return (
    <Card icon="lightbulb" title="Jaz Mirabel Beach — verified facts" sub="Product master · structured amenity record" badges={<span className="eb-badge ok"><Ki name="checkmark-filled" size={11} /> Source-backed</span>}>
      <div className="eb-amens">
        {h.amenities.map((a: string) => <span className="eb-amen hi" key={a}>{a} ✓</span>)}
      </div>
...
export function CompareGrid({ onSelect, busy }: { onSelect: (id: string) => void; busy: boolean }) {
  return (
    <div className="eb-compare">
      {[offers.coral, offers.redsea, offers.sinai].map((o: any) => (
        <div className={`eb-offer ${o.rank === 'Best match' ? 'best' : ''}`} key={o.offerId}>
          ...
          <h4>{o.hotelName}</h4>
          <div className="price">{fmtGBP(o.price)} <small>family of 3 · 7 nights</small></div>
          <div className="eb-match"><span>Match</span><span className="bar"><i style={{ width: o.match + '%' }} /></span><b>{o.match}%</b></div>
          <div className="eb-amens">{o.amenities.slice(0, 4).map((a: string) => <span key={a} className={`eb-amen ${(o.hi || []).includes(a) ? 'hi' : ''}`}>{a}</span>)}</div>
          <ul className="eb-whys">{o.matchReasons.map((m: string) => <li key={m}>{m}</li>)}</ul>
...
export function ProposalCard({ versions, activeId, onSetActive, addons }: any) {
  const v = versions.find((x: any) => x.versionId === activeId) || versions[versions.length - 1];
  const o = offers[v.offerId];
  return (
    <Card icon="document-chart" title="Proposal" sub="Composed by the Proposal Composer · review before sharing" badges={<><span className="eb-badge ok"><Ki name="checkmark-filled" size={11} /> Policy validated</span><span className="eb-badge info">{v.status}</span></>}>
      ...
      <div className="eb-seg"><span className="ic"><Ki name="arrow-up-right" size={14} /></span><div><div className="t">Flight · Airline Partner</div><div className="s">London LHR → Sharm el-Sheikh · {v.dates}</div></div><span className="p"><small>included</small></span></div>
      <div className="eb-seg"><span className="ic"><Ki name="dashboard" size={14} /></span><div><div className="t">{o.hotelName}</div><div className="s">7 nights · all inclusive · family room</div></div><span className="p">{fmtGBP(o.price)}</span></div>
      {addons.map((a: any) => (
        <div className="eb-seg" key={a.id}><span className="ic"><Ki name="recommend" size={14} /></span><div><div className="t">{a.name}</div><div className="s">Add-on · {a.why}</div></div><span className="p">+{fmtGBP(a.price)}</span></div>
      ))}
      <div className="eb-totals" style={{ marginTop: 12 }}>
        <div className="row"><span>Total client price</span><b style={{ fontFamily: 'var(--font-head)', fontSize: 17 }}>{fmtGBP(v.total)}</b></div>
        <div className="row"><span>Agency commission · Gold 12%</span><b>{fmtGBP(Math.round(v.total * 0.12))}</b></div>
        {v.delta && <div className="row"><span>Change vs {v.delta.vs}</span><span className={`eb-delta ${v.delta.amount < 0 ? 'down' : 'up'}`}>{v.delta.amount < 0 ? '−' : '+'}{fmtGBP(Math.abs(v.delta.amount))} · {v.delta.reason}</span></div>}
      </div>
...
export function DiffCard({ onApprove, onRevert, busy }: { onApprove: () => void; onRevert: () => void; busy: boolean }) {
  return (
    <Card icon="analytics" title="Your change is ready — review the diff" sub="Modification & Reprice skill · V2 draft vs V1 (held)" badges={<span className="eb-badge warn"><Ki name="warning-alt" size={11} /> Awaiting your approval</span>}>
      <table className="eb-diff">
        ...
        <tbody>
          ...
          <tr className="changed"><td className="lbl">Total price</td><td>{fmtGBP(3465)}</td><td><b>{fmtGBP(3365)}</b> <span className="eb-delta down">−£100 · flight fare lower</span></td></tr>
          <tr><td className="lbl">Hotel</td><td colSpan={2}>Coral Bay Family Resort — unchanged, amenities identical</td></tr>
          <tr><td className="lbl">Flight route</td><td colSpan={2}>London LHR → Sharm · Airline Partner — same routing, earlier week</td></tr>
...
export function HoldCard({ version, total }: { version: string; total: number }) {
  return (
    <Card icon="checkmark-filled" title="Hold placed successfully" sub="Hold/Servicing Executor · confirmed via booking platform" badges={<span className="eb-badge ok">Self-service · no support contact</span>}>
      <div className="eb-holdref">
        ...
        <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-head)', fontSize: 18 }}>{fmtGBP(total)}</span>
      </div>
...
export function DeflectionBanner({ items }: { items: string[] }) {
  return (
