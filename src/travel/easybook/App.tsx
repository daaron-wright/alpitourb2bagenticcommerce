/* ============================================================
   ...
   Migrated from travel/easybook/app.jsx
   ============================================================ */
import React, { useState, useRef } from 'react';
import { fmtGBP, dashboard, needs, blockers as defaultBlockers, STAGE_LABEL, offers } from './data';
import {
  Ki, IntakeCard, FactsCard, SoldOutCard, CompareGrid,
  ProposalCard, DiffCard, HoldCard, DeflectionBanner, SummaryRail,
} from './Components';
import { OrchestrationRail } from './Rail';
...
export function App() {
  ...
  const [blk, setBlk] = useState(defaultBlockers);
  ...
  const showToast = (t: string) => { setToast(t); clearTimeout(toastT.current); toastT.current = setTimeout(() => setToast(null), 2800); };
  const pushAgent = (html: string, skill?: string) => setMsgs((m) => [...m, { role: 'agent', html, skill }]);
  const pushUser = (html: string) => setMsgs((m) => [...m, { role: 'user', html }]);
  const logEv = (actor: string, summary: string, reversible?: boolean) => setTimeline((t) => [...t, { ts: stamp(), actor, summary, reversible }]);
  const addSrc = (ids: string[]) => setSrcIds((s) => [...s, ...ids.filter((i) => !s.includes(i))]);

  function orchestrate({ user, status, delay = 1000, then }: { user?: string; status?: string; delay?: number; then: () => void }) {
    if (user) pushUser(user);
    setQuick([]);
    setWorking(status || 'Travel Assistant is working…');
  ...
  function openLead() {
    ...
    tick = 0;
    setMsgs([{ role: 'agent', html: "I've read the Family A brief and structured it into requirement chips — see the left rail. <b>Two questions still block the search</b>: departure airport, and date flexibility if their hotel is unavailable. Tap them to resolve, or tell me.", skill: 'Requirement Extractor' }]);
    setTimeline([]); setSrcIds([]); setVersions([]); setActiveVer(null); setSelected(null); setSignals([]); setBlk(defaultBlockers);
    logEv('user', 'Opened seeded work package WP-2231 (Traveler Family A)');
  ...
  function selectOffer(id: string) {
    setSelected(id);
    orchestrate({
      user: `Select ${offers[id].hotelName}.`, status: 'Composing the package and drafting proposal V1…', delay: 1250,
      then: () => {
        ...
        pushAgent(`Proposal <b>V1 is drafted</b> — ${offers[id].hotelName}, insurance and private transfer included: <b>${fmtGBP(3465)}</b>, your commission ${fmtGBP(416)}. Review it in the workspace. Placing a hold is a commercial commit, so <b>it needs your approval</b>.`, 'Proposal Composer');
        setApproval({ actionType: 'place hold', summary: 'Lock Coral Bay + Airline Partner under proposal V1 until tomorrow 18:00. Reversible — you can release the hold anytime.' });
  ...
  function onEscalate(mode?: string) {
  ...
  const planIdx: [number, number] = ({ intake: [-1, 0], clarifying: [-1, 0], enriching: [0, 1], availability_check: [1, 2], sold_out: [3, 4], alternatives_ready: [4, 5], proposal_ready: [5, 6], hold_confirmed: [6, 7], v2_ready: [6, 7], completed: [7, -1] } as any)[stage] || [-1, 0];
  ...
  return (
    <div className="eb-app">
      <header className="eb-top">
        <div className="eb-logo"><span className="mark">E</span><span className="t">EasyBook Next <small>· Travel Assistant embedded</small></span></div>
        <div className="eb-crumb">
          ...
          {route === 'workbench' && <><span className="sep">/</span><b>WP-2231 · Traveler Family A</b></>}
        </div>
        <div className="eb-top-r">
          {route === 'workbench' && <span className="eb-stage"><span className={`dot ${working ? 'busy' : ''}`} /> {STAGE_LABEL[stage]}</span>}
          <div className="eb-agency"><span>Partner Travel Agency</span><span className="gold">Gold</span></div>
          ...
          <div className="eb-dash-in">
            ...
            <div className="eb-dash-grid">
              <div className="eb-panel">
                ...
                {dashboard.resume.map((w: any) => (
                  <div className={`eb-wp ${w.hot ? 'hot' : ''}`} key={w.id} onClick={() => w.hot ? openLead() : showToast('This prototype follows WP-2231 — open the Family A lead.')}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div className="eb-panel">
                  ...
                  {dashboard.signals.map((s: any, i: number) => (
