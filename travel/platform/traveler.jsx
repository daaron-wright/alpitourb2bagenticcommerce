/* ============================================================
   Travel Group Platform · traveler view (Consumer Travel Portal) — English
   Airbnb-pattern, Shidoka-skinned. Travel Assistant sheet carries the
   B2B2C journey: inquiry → 7-question needs capture (chips OR
   free text, plus a multimodal photo moment) → travel-profile
   cluster card (1 of 8) → recommendation confirmation → handoff
   → support. Live two-way thread via window.PlatformBus.
   ============================================================ */
(function () {
  const { useState, useEffect, useRef } = React;
  const { categories, packages, agency, visualMatch } = window.TV;
  const bus = window.PlatformBus;
  ...
  /* ---------- header ---------- */
  function THeader({ onAssistant }) {
    return (
      <header className="tv-head" data-screen-label="Consumer Travel Portal header">
        <div className="tv-logo"><span className="mark">A</span><div><b>Consumer Travel Portal</b><i>Client Travel Group</i></div></div>
  ...
  /* ---------- home — one search window, ChatGPT-style ---------- */
  function THome({ onSubmit }) {
      ...
      <section className="tv-home" data-screen-label="Search window">
        ...
        <div className="ask-box">
          <input value={q} onChange={(e) => setQ(e.target.value)} autoFocus
            ...
            placeholder="Tell me about the holiday…" aria-label="Ask Travel Assistant" />
  ...
  /* ---------- trip status — after handoff, fed by the agent's work ---------- */
  function TTrip({ b, onAssistant }) {
    const steps = [
      { done: true, t: "Request sent to Partner Travel Agency", s: "Your answers, your profile and a 24h price lock — nothing to repeat" },
    ...
    const agentFeed = (b.feed || []).filter((f) => f.who === "agent");
    return (
      <section className="tv-trip" data-screen-label="Trip status">
        ...
        {agentFeed.length > 0 && (
          <div className="feed" data-screen-label="Live updates from the agency">
            ...
            {agentFeed.slice().reverse().map((f, i) => (
  ...
  /* ---------- agentic search card — visible tool steps ---------- */
  const AGENTIC_STEPS = [
  ...
  /* ============================================================
     Travel Assistant sheet — the B2B2C conversation
     ============================================================ */
  const QSEQ = [
    ...
    { id: "from", q: "Last one: where do you fly from?", chips: ["London Heathrow", "London Gatwick"], k: "Departure" },
  ];

  function TSheet({ open, onClose, b, inline, boot }) {
    const [msgs, setMsgs] = useState([]);
    const [typing, setTyping] = useState(false);
    const [awaiting, setAwaiting] = useState(null); // index into QSEQ, or null
    const [draft, setDraft] = useState("");
    const [photoCap, setPhotoCap] = useState("");
    const answers = useRef([]);
    const started = useRef(false);
    const bodyRef = useRef(null);
    const push = (m) => setMsgs((x) => [...x, ...(Array.isArray(m) ? m : [m])]);
    const later = (ms, fn) => { setTyping(true); setTimeout(() => { setTyping(false); fn(); }, ms); };
    ...
    useEffect(() => {
      ...
      if (boot && boot.type === "text") {
        ...
        bus && bus.push("feed", { who: "traveler", t: `Opened a request: “${boot.text}”` });
        later(1200, () => askQuestion(0));
        return;
      }
      if (boot && boot.type === "chip") {
        push({ kind: "ai", html: "Hello! I'm <b>Travel Assistant</b>. Tell me about the holiday in your own words — I'll build the request for your travel agency." });
        setTimeout(() => onChip({ id: boot.id }), 250);
        return;
      }
      push({ kind: "ai", html: "Hello! I'm <b>Travel Assistant</b>. Tell me about the holiday in your own words — I'll build the request for your travel agency.", chips: starterChips() });
    }, [open]);

    function starterChips(except) {
      return [
    ...
    function askQuestion(idx) {
      ...
      const st = QSEQ[idx];
      push({ kind: "ai", html: `<span class="qn num">${idx + 1}/7</span> ${st.q}`, chips: st.chips.map((label, i) => ({ id: `${st.id}:${i}`, label })) });
      setAwaiting(idx);
    }
    ...
    function answer(idx, text) {
      ...
      bus && bus.push("feed", { who: "traveler", t: `${st.k}: ${text}` });
      later(900, () => {
        if (st.photo) {
    ...
    function photoDone(skipped) {
      clearChips();
      setMsgs((x) => x.map((m) => (m.kind === "photo" ? { ...m, locked: true } : m)));
      const beenIdx = QSEQ.findIndex((s) => s.photo);
      ...
      bus && bus.push("feed", { who: "traveler", t: "Sent a photo with the request — Travel Assistant is reading the preferences" });
      later(1600, () => {
    ...
    function confirmVisual() {
      ...
      bus && bus.push("feed", { who: "traveler", t: "Visual preference confirmed: a kids' lagoon like the photo" });
      const beenIdx = QSEQ.findIndex((s) => s.photo);
    ...
    function confirmNeeds() {
      ...
      bus && bus.emit("brief", { answers: answers.current, cluster: "fam", conf: 96 });
      bus && bus.push("feed", { who: "traveler", t: "Request confirmed — Beach & family profile (1 of 8)" });
      setTimeout(() => push({ kind: "agentic" }), 700);
      setTimeout(() => { bus && bus.push("feed", { who: "traveler", t: "Travel Assistant agentic search · 142 stays → 12 match → 3 fit the family profile" }); }, 3200);
      setTimeout(() => {
        push({ kind: "ai", html: "Done — and from your answers I recognize you right away. This is your travel profile, the same one that personalizes Consumer Travel Portal for you:" });
        push({ kind: "profile" });
        push({ kind: "reco" });
      }, 5200);
    }
    ...
    function sendToAgency() {
      ...
      later(1400, () => {
        bus && bus.emit("handoff");
        bus && bus.push("feed", { who: "traveler", t: "Request sent to Partner Travel Agency — with a 24h price lock" });
        push({ kind: "handoff" });
        push({ kind: "ai", html: `That's everything from my side — <b>your request is no longer with me or Consumer Travel Portal: it's with Partner Travel Agency now</b>. ${agency.agent} has your answers, your profile, the photo and the locked price, and she takes it from here — checking real availability, holding the trip and sending your proposal. <b>There's nothing you need to do.</b> I'll stay here for questions, and you'll see every step from her in your trip page.`, chips: supportChips() });
      });
    }
    ...
    function onChip(c) {
      if (c.id === "inq") {
        ...
        bus && bus.push("feed", { who: "traveler", t: "Opened a request: Egypt, August, all inclusive" });
        later(1200, () => askQuestion(0));
        return;
      }
      ...
      if (c.id === "q-warm") { clearChips(); push({ kind: "user", html: "Where's good for a toddler in August?" }); later(1000, () => push({ kind: "ai", html: "With a 2-year-old in August I'd look at the <b>Red Sea</b> — Sharm el-Sheikh above all: a short direct flight from London, shallow sandy lagoons and resorts built around families. Tell me about your trip and I'll put the request together:", chips: starterChips("q-warm") })); return; }
      ...
      if (c.id === "q-prop") { clearChips(); push({ kind: "user", html: "What does the proposal include?" }); later(900, () => push({ kind: "ai", html: "A direct Airline Partner flight from London, a family room all inclusive, family insurance and a transfer with a child seat — with the price verified live. The full page is in your <b>Your trip</b> section.", chips: supportChips() })); return; }
      ...
      <>
            ...
            {msgs.map((m, i) => {
                  ...
                  {!m.locked && (
                    <>
                      ...
                      <div className="chips" style={{ display: "flex", gap: 7, marginTop: 3 }}>
                        <button className="tv-btn primary sm" onClick={() => photoDone(false)}>Send to Travel Assistant</button>
              ...
              if (m.kind === "profile") return (
                <div className="card" key={i} data-screen-label="Travel profile cluster">
                  ...
                  <div className="rs"><b>Beach & family</b> — seaside holidays built around little ones. Looked after by <b>Family Travel</b>, the Travel Group world for families: it's the same profile your agency receives with the request.</div>
                </div>
              );
              ...
              return (
                <div className="m ai" key={i}>
                  ...
                  {m.chips && <div className="chips">{m.chips.map((c) => <button key={c.id} className={`tv-btn sm ${c.kind === "primary" ? "primary" : ""}`} onClick={() => onChip(c)}>{c.label}</button>)}</div>}
                </div>
              );
            })}
          ...
          <div className="sc">
            <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendDraft()}
              placeholder={awaiting !== null ? "…or answer in your own words" : "Write to Travel Assistant…"} aria-label="Write to Travel Assistant" />
            <button onClick={sendDraft} aria-label="Send"><Ki name="arrow-up-right" size={14} /></button>
          </div>
      </>
    );
    if (inline) return <section className="tv-sheet inline" data-screen-label="Travel Assistant chat" aria-label="Travel Assistant">{inner}</section>;
    return (
      <div className="tv-scrim" onClick={onClose}>
        <aside className="tv-sheet" onClick={(e) => e.stopPropagation()} data-screen-label="Travel Assistant sheet" aria-label="Travel Assistant">{inner}</aside>
      </div>
    );
  }

  /* ---------- traveler app — home search window → chat page ---------- */
  function TravelerApp() {
    const [b, setB] = useState(bus ? { ...bus.state } : {});
    const [view, setView] = useState(bus && bus.state.handoff ? "chat" : "home");
    const [boot, setBoot] = useState(null);
    useEffect(() => bus && bus.on((s) => setB({ ...s })), []);
    const startChat = (bt) => { setBoot(bt); setView("chat"); };
    return (
      <div className="tv-app">
        <THeader onAssistant={() => setView("chat")} />
        {view === "home" ? (
          <>
            <THome onSubmit={startChat} />
            <footer className="tv-foot">Client Travel Group — Travel Group · Premium Travel · Family Travel · Value Travel · Signature Travel. Requests travel to your trusted agency.</footer>
          </>
        ) : (
          <div className={`tv-chatwrap ${b.handoff ? "with-trip" : ""}`} data-screen-label="Traveler chat page">
            {b.handoff && <TTrip b={b} onAssistant={() => {}} />}
  ...
  ReactDOM.createRoot(document.getElementById("b2c-root")).render(<TravelerApp />);
})();
