/* ============================================================
   EasyBook Next — right orchestration rail (Chat · Plan ·
   Sources · Timeline · Support) + approval banner
   ============================================================ */
(function () {
  const { useEffect, useRef, useState } = React;
  const { receipts, PLAN } = window.EB;

  function OrchestrationRail({ tab, setTab, msgs, working, quick, onQuick, onSend, approval, onApprove, onReject, planDoneIdx, planActiveIdx, confidence, timeline, srcIds, escalation, onEscalate, counts }) {
    const TABS = [["chat", "Chat"], ["plan", "Plan"], ["sources", "Sources"], ["timeline", "Timeline"], ["support", "Support"]];
    return (
  ...
  function ChatTab({ msgs, working, quick, onQuick, onSend }) {
      ...
      <>
        ...
        {quick.length > 0 && !working && (
          <div className="eb-quick">
            {quick.map((q) => <button key={q.id} className={q.kind || ""} onClick={() => onQuick(q)}>{q.label}</button>)}
          </div>
        )}
        <div className="eb-composer">
          <input className="eb-input" placeholder="Ask, or describe the change…" value={text} disabled={!!working}
            onChange={(e) => setText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(); }} aria-label="Message Travel Assistant" />
  ...
  function PlanTab({ doneIdx, activeIdx, confidence }) {
    return (
      <>
  ...
  function SourcesTab({ ids }) {
    if (!ids.length) return <div className="eb-src"><div className="eb-empty-note">Source receipts appear here as Travel Assistant retrieves facts, availability and pricing.</div></div>;
    return (
  ...
  function SupportTab({ escalation, onEscalate }) {
    return (
      <div className="eb-esc">
        <div className="eb-esc-note">
          {escalation.blocked
            ...
            : <>Nothing is blocked right now. If something does block, Travel Assistant prepares a complete escalation packet — context, timeline and sources included — instead of a generic "contact support" link.</>}
        </div>
  ...
  Object.assign(window, { OrchestrationRail });
})();
