/* ============================================================
   Travel Assistant B2B Concierge — workspace cards
   All components exported to window at the end (multi-Babel-file rule).
   ============================================================ */
(function () {
  const { fmtGBP } = window.CG;
  ...
  function Card({ icon, title, sub, badges, children, className = "" }) {
    return (
  ...
  function WelcomeCard() {
    return (
      <div className="cg-card cg-welcome">
        <span className="cg-eyebrow">EasyBook · Travel Assistant concierge</span>
        ...
        <div className="caps">
          {[["chat-bot", "Build", "Brief → ranked options → priced package"], ["analytics", "Check", "Availability, amenities & policy answers"], ["recommend", "Modify", "Self-service changes after proposal"], ["checkmark-filled", "Hold", "24-hour booking holds from the chat"]].map(([ic, t, s]) => (
    ...
    return (
      <Card icon="document-chart" title="Request captured" sub="Work package wpkg_travel_001 · classified standard_package_quote" badges={<span className="cg-badge ai"><Ki name="recommend" size={11} /> Intent Agent</span>}>
        <dl className="cg-kv">
          ...
          <dt>Budget</dt><dd>{fmtGBP(r.budget)}</dd>
  ...
  /* ---------- trip context (full + strip) ---------- */
  function TripContextCard({ signals }) {
    const r = window.CG.requirements;
    return (
  ...
  /* ---------- 3 · agency context ---------- */
  function AgencyContextCard() {
    const a = window.CG.agency;
    return (
      <>
        ...
        <Card icon="warning-alt" title="Commercial rules" sub="OPA policy guardrails · enforced at proposal time">
          <div className="cg-amens" style={{ gap: 7 }}>
            ...
            <span className="cg-amen">Brand template · Travel Group</span>
  ...
  /* ---------- 4 · hotel knowledge (retrieval) ---------- */
  function HotelKnowledgeCard() {
  ...
  function HotelOptionCard({ hotel, onSelect, busy }) {
    ...
    return (
      <div className={`cg-opt ${best ? "best" : ""}`}>
        ...
        <h4>{hotel.name}</h4>
        <div className="price">{fmtGBP(hotel.price)} <small>family of 3 · 7 nights</small></div>
        <div className="cg-match"><span>Match</span><span className="bar"><i style={{ width: hotel.matchScore + "%" }} /></span><b>{hotel.matchScore}%</b></div>
        <div className="cg-amens">{hotel.amenities.slice(0, 4).map((a) => <span key={a} className={`cg-amen ${(hotel.hi || []).includes(a) ? "hi" : ""}`}>{a}</span>)}</div>
  ...
  function AlternativesGrid({ onSelect, busy }) {
  ...
  /* ---------- 7 · package builder ---------- */
  function PackageBuilder({ hotel, addOns, compact, onEdit }) {
    const F = window.CG.flight;
    const segs = [
      ["arrow-up-right", "Flight · " + F.carrier, `${F.route} · ${F.dates} · ${F.code}`, "included"],
      ["dashboard", hotel.name, "7 nights · all-inclusive · family room", fmtGBP(hotel.price)],
    ...
    const addSegs = window.CG.addOns.filter((a) => addOns.includes(a.id) && a.id !== "transfer");
    return (
      <Card icon="document-chart" title="Package summary" sub="Composed by the Itinerary Agent · availability confirmed" badges={<span className="cg-badge ok"><Ki name="checkmark-filled" size={11} /> Flight + hotel confirmed</span>}>
        ...
        {addSegs.map((a) => (
          <div className="cg-seg" key={a.id}>
            ...
            <span className="p">+{fmtGBP(a.price)}</span>
          </div>
        ))}
        {!compact && (
          <div className="cg-edits">
            {[["Change hotel", "hotel"], ["Change dates", "dates"], ["Modify flight", "flight"], ["Add experiences", "addons"]].map(([l, k]) => (
          ...
          {window.CG.addOns.map((a) => {
            ...
            return (
              <div key={a.id} className={`cg-addon ${sel ? "sel" : ""}`} onClick={() => onToggle(a.id)}>
                ...
                <div style={{ marginLeft: "auto", textAlign: "right" }}><div className="p">+{fmtGBP(a.price)}</div></div>
              </div>
            );
          })}
        </div>
  ...
  /* ---------- 9 · proposal preview ---------- */
  function ProposalPreview({ hotel, addOns, totals, onHold, onRevise, busy, version, dates }) {
    const F = window.CG.flight;
    const adds = window.CG.addOns.filter((a) => addOns.includes(a.id));
    return (
      <Card icon="document-chart" title={`Proposal ${version || "v1"} · Partner Travel Agency`} sub="Generated by the Proposal Agent · Travel Group brand template"
        ...
        <div className="cg-proposal-grid">
          <dl className="cg-kv" style={{ alignContent: "start" }}>
            ...
            <dt>Add-ons</dt><dd>{adds.length ? adds.map((a) => a.name).join(", ") : "None"}</dd>
          </dl>
          <div>
            <div className="cg-totals">
              <div className="row"><span>Package price</span><b>{fmtGBP(totals.base)}</b></div>
              <div className="row"><span>Add-ons</span><b>{totals.addons ? "+" + fmtGBP(totals.addons) : "—"}</b></div>
              <div className="row grand"><span>Total client price</span><b>{fmtGBP(totals.total)}</b></div>
            </div>
            <div className="cg-commission" style={{ marginTop: 12 }}>
              <span>Agency commission · 12%</span><b>{fmtGBP(totals.commission)}</b>
            </div>
  ...
  /* ---------- 10 · booking hold ---------- */
  function BookingHoldCard({ version, total, onAction, busy }) {
    return (
      <Card icon="checkmark-filled" title="Booking hold confirmed" sub="Booking Agent · booking-platform-mcp → EasyBook" badges={<span className="cg-badge warn"><Ki name="warning-alt" size={11} /> On hold · expires in 24h</span>}>
        <div className="cg-holdref">
          ...
          <span style={{ marginLeft: "auto", fontFamily: "var(--font-display)", fontSize: 20, fontVariantNumeric: "tabular-nums" }}>{fmtGBP(total)}</span>
        </div>
        <div className="cg-actions-grid">
          {[["checkmark-filled", "Confirm booking", "confirm"], ["recommend", "Modify proposal", "modify"], ["arrow-up-right", "Send to customer", "send"], ["information", "Add follow-up reminder", "remind"]].map(([ic, l, k]) => (
  ...
  /* ---------- 11 · modification panel ---------- */
  function ModificationPanel({ onPick, busy }) {
    return (
      <Card icon="recommend" title="Modify proposal — self-service" sub="No support call, no ticket · Modification Agent re-orchestrates the pipeline">
        <div className="cg-modopts">
          {[["analytics", "Change dates", "dates"], ["arrow-up-right", "Modify flight departure", "flight"], ["dashboard", "Switch hotel", "hotel"], ["recommend", "Add / remove services", "services"]].map(([ic, l, k]) => (
  ...
  /* ---------- 11b · version compare ---------- */
  function VersionCompareCard({ onAccept, onKeep, busy }) {
    return (
      <Card icon="analytics" title="Version compare" sub="Same hotel & flight route available one week earlier · re-priced by the Pricing Agent">
        <div className="cg-vers">
          <div className="cg-ver">
            ...
            <div className="vtotal">{fmtGBP(3465)}</div>
          ...
          <div className="cg-ver new">
            ...
            <div className="vtotal">{fmtGBP(3365)} <span className="cg-delta down"><Ki name="arrow-down-right" size={12} /> −£100</span></div>
  ...
  /* ---------- 12 · hotel switch ---------- */
  function HotelSwitchCard({ onApply, onKeep, busy }) {
    const h = window.CG.hotels.sinai;
    return (
        ...
        <div style={{ display: "flex", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginTop: 12 }}>
              <span style={{ fontFamily: "var(--font-display)", fontSize: 22 }}>{fmtGBP(3795)}</span>
  ...
  /* ---------- 13 · NBA + business value ---------- */
  function NextBestActionsPanel({ onAction }) {
    return (
      <Card icon="lightbulb" title="Next best actions" sub="Upsell Agent · contextual follow-ups for this booking">
        <div className="cg-nba">
          {[["arrow-up-right", "Send updated proposal to client", "send"], ["information", "Set follow-up reminder · 48h", "remind"], ["checkmark-filled", "Confirm booking", "confirm"], ["recommend", "Add water park day pass · £160", "wp"], ["dashboard", "Add room upgrade · £280", "ru"], ["group", "Save family preference for future proposals", "save"]].map(([ic, l, k]) => (
  ...
  Object.assign(window, {
    Ki, CgCard: Card, WelcomeCard, RequestCapturedCard, TripContextCard, TripStrip,
    AgencyContextCard, HotelKnowledgeCard, AvailabilityCard, AlternativesGrid,
    PackageBuilder, AddOnsPanel, ProposalPreview, BookingHoldCard,
    ModificationPanel, VersionCompareCard, HotelSwitchCard,
    NextBestActionsPanel, BusinessValueSummary,
  });
})();
