/* ============================================================
   EasyBook Next — B2B Storefront · Travel Concierge orchestration rail
   Tabs: Chat · Plan · Sources · Timeline. Approval gates live here.
   ============================================================ */
(function () {
  const { receipts, orchestrator } = window.SF;
  ...
  function ChatInquiry({ done, onAccept }) {
    return (
      <div className={`sf-chatinq ${done ? "done" : ""}`} data-screen-label="Incoming brief">
        <div className="ci-top"><span className="ci-pill">United Kingdom · Family Travel</span><span className="ci-hold">24h hold</span></div>
        <div className="ci-row">
          <span className="ci-av" aria-hidden="true">C</span>
          <div className="ci-id"><b>Traveler Family A</b><i>London · Bath · Cornwall · from Toronto</i></div>
        </div>
        <div className="ci-facts"><span>12–22 Aug</span><span>≤ CA$11,000</span></div>
        <div className="ci-foot">
          <span className="ci-src"><SfInf size={11} /> via Travel Concierge · Consumer Travel Portal</span>
          {done
  ...
  function Rail({ open, setOpen, tab, setTab, msgs, working, quick, onQuick, onSend, approval, onApprove, onReject, planDone, planActive, confidence, srcIds, timeline, asb, onToast, brand, onPickBrand, live, liveSticky, request, onPickRequest, choices, jstep, onAcceptInquiry, inquiryDone }) {
    const agent = brand === "signature" ? "Travel Orchestrator" : "Travel Concierge";
              ...
              <div className="sf-msgs">
                ...
                {msgs.map((m, i) => (
                  m.role === "user"
                    ...
                    : m.role === "tool"
                    ? <ChatFold key={i} icon={m.t.status === "err" ? "error-filled" : "ai"} label={m.t.title || "Tool call"} meta={m.t.tool}><SfToolCallCard t={m.t} /></ChatFold>
                    : m.role === "attachment"
                    ...
                    : m.role === "handoff"
                    ? <ChatFold key={i} icon="ai" label={m.h.dir === "in" ? "Handoff received" : "Handoff sent"} meta={m.h.meta}><SfHandoffCard h={m.h} /></ChatFold>
                    : m.role === "inquiry"
          ...
          {tab === "live" && (
            <div className="sf-livetab" data-screen-label="Live updates tab">
              ...
              {live && live.length > 0
                ...
                : <div className="sf-empty">No live updates yet. Price, allotment and catalogue events stream here as Travel Concierge watches the trip — they never clutter the conversation.</div>}
            </div>
          )}
          ...
          {tab === "plan" && (
            <div className="sf-plan">
              <div className="sf-jtab">
                <div className="uc"><b>{orchestrator.useCase.title}</b><span>One seamless touchpoint — the agent works, the customer experiences.</span></div>
