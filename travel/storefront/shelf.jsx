/* ============================================================
   ...
   ============================================================ */
(function () {
  const { fmtGBP, brands, products, SHELVES, BRIEF, chips, desk } = window.SF;
  const brandOf = (id) => brands.find((b) => b.id === id);

  /* ---------- product card ---------- */
  function ProductCard({ p, ranked, soldout, inCompare, onCompare, onOpen, onSelect, onSource, busy }) {
    ...
    return (
      <article className={`sf-card ${soldout ? "soldout" : ""} ${ranked && p.rank === "Best match" ? "best" : ""}`}>
        ...
        <div className="body">
          ...
          <div className="dest"><SfStars n={p.rating} /> {p.dest} · {p.board} · {p.nights} nights</div>
          <div className="dest"><SfAllot a={p.allotment} /><SfSrcChip ids={p.receipts} onSource={onSource} /></div>
          {p.expiry && !ranked && <div className="dest"><SfBadge kind="warn" icon="warning-alt">{p.expiry}</SfBadge></div>}
          <div className="ams">
            {p.amenities.map((a) => <span key={a} className={`sf-am ${ranked && p.hi && p.hi.includes(a) ? "hi" : ""}`}>{a}</span>)}
          </div>
          {ranked && p.matchReasons && (
            <div className="why">{p.matchReasons.map((r, i) => <span key={i}>{i > 0 && " · "}{i === 0 ? <b>{r}</b> : r}</span>)}</div>
          )}
          <div className="sf-commerce">
            {soldout ? (
              ...
              <div><span className="gross num">{fmtGBP(p.price)}</span><span className="per">family 2+1 · flight incl.</span></div>
            )}
            {!soldout && (
              <div className="agentline sf-agent-only">
                <div className="comm num">{fmtGBP(p.commission)} comm. · {p.commissionPct}%</div>
                <div className="net num">net {fmtGBP(p.net)}</div>
              </div>
            )}
          </div>
        </div>
        <div className="foot">
          ...
          {!soldout && ranked && <button className="sf-btn sm primary" style={{ marginLeft: "auto" }} disabled={busy} onClick={() => onSelect(p.id)}>Select for package</button>}
  ...
  /* ---------- brand band ---------- */
  function BrandBand({ filter, setFilter }) {
    return (
      <section className="sf-brandband" aria-label="Client Travel Group brands">
  ...
  /* ---------- shelf row ---------- */
  function ShelfRow({ shelf, filter, compareIds, onCompare, onOpen, onSource }) {
    const items = Object.values(products).filter((p) => (p.shelf || []).includes(shelf.id) && (!filter || p.brand === filter));
  ...
  /* ---------- customer reference — collapsible, human-centered ----------
     The end customer (the Traveler Family A) lives here: who they are, their
     own words, what matters, and what they did. Collapsed by default;
     auto-opens only while there are questions to confirm. */
  function ChipsBar({ blockers, onFix, stage }) {
    const rq = ((window.SF.orchestrator && window.SF.orchestrator.requests) || []).find((r) => r.id === "travelerA") || {};
    ...
    return (
      <div className={`sf-cust ${expanded ? "open" : ""}`} data-screen-label="Customer brief">
        <button className="sf-cust-bar" onClick={() => setOpen((o) => !o)} aria-expanded={expanded}>
          ...
          <div className="who">
            <b>Traveler Family A</b>
            <i>Two parents and their 2-year-old, from Toronto · 10 slow days around the United Kingdom</i>
          </div>
          <span className="src"><SfInf size={11} /> via Travel Concierge · Consumer Travel Portal</span>
            ...
            <div className="sf-cust-grid">
              <div className="col">
                ...
                <div className="warmrow">
                  {(rq.signals || []).map((s, i) => <span key={i} className="warm">{s}</span>)}
                </div>
              </div>
              <div className="col">
                <h5>The details, structured</h5>
                <div className="chiprow">
                  {chips.map((c) => <span key={c.id} className="sf-chip fixed"><Ki name={c.icon} size={12} /> {c.label}</span>)}
                </div>
  ...
  function SoldOutBanner() {
    return (
      <div className="sf-soldout" data-screen-label="Sold-out recovery">
        ...
        <div>
          <div className="t">The St Ives Signature is sold out for 12–22 August — live inventory, checked 43s ago.</div>
          <div className="s">Travel Concierge kept every hard constraint — three cities, family rooms, air-con, short transfers, ≤ £7,400 — and ranked the three closest alternatives below. Family rooms and short transfers were weighted first because of the 2-year-old.</div>
        </div>
      </div>
    );
  }
  ...
  function CompareTray({ ids, onOpen, onClear }) {
    if (ids.length < 1) return null;
    const names = ids.map((i) => products[i].name);
    return (
  ...
  function CompareOverlay({ ids, onClose, onSelect, canSelect, onSource }) {
    const items = ids.map((i) => products[i]);
    const bestPrice = Math.min(...items.map((p) => p.price));
    const bestComm = Math.max(...items.map((p) => p.commission));
    const row = (label, render, bestOf) => (
      <tr><th>{label}</th>{items.map((p) => <td key={p.id} className={bestOf && bestOf(p) ? "best" : ""}>{render(p)}</td>)}</tr>
    );
    return (
        ...
        <div className="sf-compare" onClick={(e) => e.stopPropagation()} data-screen-label="Compare overlay">
          ...
          <table className="sf-cmp-table">
            <tbody>
              ...
              {row("Gross · family 2+1", (p) => <b className="num">{fmtGBP(p.price)}</b>, (p) => p.price === bestPrice)}
              {row("Your commission", (p) => <b className="num sf-agent-only" style={{ color: "var(--k-spruce-70)" }}>{fmtGBP(p.commission)} · {p.commissionPct}%</b>, (p) => p.commission === bestComm)}
              {row("Board · nights", (p) => `${p.board} · ${p.nights} nights`)}
              {row("Allotment", (p) => <SfAllot a={p.allotment} />)}
              {row("Family fit", (p) => (
                <div className="ams">{p.amenities.map((a) => <span key={a} className={`sf-am ${p.hi && p.hi.includes(a) ? "hi" : ""}`}>{a}</span>)}</div>
              ))}
              {row("Match to brief", (p) => p.match ? <SfBadge kind={p.rank === "Best match" ? "ai" : "neutral"}>{p.match}% · {p.rank}</SfBadge> : <span style={{ color: "var(--fg-muted)" }}>—</span>)}
              {row("Sources", (p) => <SfSrcChip ids={p.receipts} onSource={onSource} />)}
              {canSelect && row("", (p) => <button className="sf-btn sm primary" onClick={() => onSelect(p.id)}>Select for package</button>)}
            </tbody>
          </table>
        </div>
      </div>
  ...
  /* ---------- desk board — the agent's worklist dashboard (home) ---------- */
  /* ---------- travel journey take-off diagram ---------- */
  function JourneyTakeoff({ awaiting, stage }) {
    ...
    const active = awaiting
      ...
      : stage === "completed"
        ? 4
        : ["held", "repricing", "v2"].includes(stage)
          ? 3
          : stage === "browse"
            ? 1
            : 2;
    const status = active === 0
      ? "Waiting for inquiry handoff"
      : active === 1
        ? "Inquiry received · ready to work"
        : active === 2
          ? stage === "proposal" ? "Proposal ready · awaiting hold approval" : "Proposal in progress · live checks running"
          : active === 3
            ? stage === "held" ? "Hold secured · expires tomorrow 18:00" : "Hold secured · change being serviced"
            : "Ready for departure · departs 15 Aug";
    ...
    const [loaderPos, setLoaderPos] = React.useState(null);
    ...
    return (
      <section className="bd-takeoff" data-screen-label="Journey take-off" ref={hostRef}>
        <div className="tk-h">
          <div className="tk-route"><span className="eyb">In focus · Traveler Family A</span><h3>Toronto YYZ <span>→</span> London · Bath · Cornwall</h3></div>
          <span className="tk-meta"><span className="dot" aria-hidden="true" /> {status}</span>
        </div>
        <svg className="tk-svg" viewBox="0 0 920 150" preserveAspectRatio="xMidYMid meet" aria-hidden="true" ref={svgRef}>
          ...
          {stages.map((s, i) => {
            ...
            if (current) return <g key={s.n} />; // rendered as the DS ai-spinner overlay
            return (
              <g key={s.n}>
        ...
        <div className="tk-stages">
          {stages.map((s) => <span key={s.n} className={`tk-st ${s.n < active ? "done" : s.n === active ? "current" : s.n === active + 1 ? "next" : ""}`}>{s.label}</span>)}
        </div>
      </section>
    );
  }

  /* ---------- desk overview — highly-visual dashboard widgets (world map, charts, budget, calendar, wishlist) ---------- */
  function OvDonut({ pct, val, label, tone }) {
  ...
  function DeskOverview({ onToast }) {
    const mapEl = React.useRef(null);
    const dests = [
      { id: "travelerA", ll: [51.507, -0.128], who: "Family A", place: "London · United Kingdom", tone: "#0F62FE" },
      { id: "travelerD", ll: [3.2, 73.0], who: "Traveler D", place: "Maldives", tone: "#7A2048" },
      { id: "travelerE", ll: [28.29, -16.62], who: "Traveler E", place: "Tenerife", tone: "#1F8A5B" },
      { id: "travelerG", ll: [57.12, -4.71], who: "Traveler G", place: "Scottish Highlands", tone: "#29707A" },
      { id: "travelerF", ll: [36.39, 25.46], who: "Traveler F", place: "Santorini", tone: "#8a6116" },
      { id: "travelerB", ll: [35.68, 139.77], who: "Couple B", place: "Japan", tone: "#FF462D" },
      { id: "travelerC", ll: [50.72, -3.53], who: "Traveler C", place: "Devon", tone: "#0F62FE" },
    ];
    React.useEffect(() => {
      if (!window.L || !mapEl.current) return;
      const L = window.L;
      ...
      dests.forEach((d) => {
        const icon = L.divIcon({ className: "", html: `<span class="ov-pin" style="background:${d.tone}"></span>`, iconSize: [16, 16], iconAnchor: [8, 8] });
        L.marker(d.ll, { icon }).addTo(map).bindTooltip(`${d.who} · ${d.place}`, { direction: "top", offset: [0, -6] });
      });
      map.fitBounds(dests.map((d) => d.ll), { padding: [34, 34] });
      setTimeout(() => map.invalidateSize(), 260);
      return () => map.remove();
    }, []);

    const budget = [
      { who: "Traveler D · Maldives", val: "£713", k: "warn", s: "hold" },
      { who: "Family A · United Kingdom", val: "£1,099", k: "ai", s: "proposal" },
      { who: "Traveler F · Santorini", val: "£406", k: "ok", s: "sent" },
      { who: "Traveler C · Devon", val: "£273", k: "okfill", s: "booked" },
    ];
    const first = new Date(2026, 7, 1).getDay();
    const evd = { 2: { c: "#1F8A5B", t: "Traveler C · Devon departs" }, 7: { c: "#E6A100", t: "Traveler D · Maldives hold ends" }, 8: { c: "#0F62FE", t: "Traveler E · Tenerife hold ends" }, 12: { c: "#FF462D", t: "Family A · United Kingdom departs" } };
        ...
        <section className="ov-card ov-cal">
          ...
          <div className="ov-calgrid">
            ...
            {Array.from({ length: 31 }, (_, i) => i + 1).map((d) => {
              const ev = evd[d];
              return (
                <button type="button" className={`ov-day ${d === 12 ? "today" : ""} ${ev ? "has" : ""}`} key={d} title={ev ? ev.t : undefined} onClick={ev ? () => onToast(ev.t) : undefined}>{d}{ev && <i style={{ background: ev.c }}></i>}</button>
              );
            })}
          </div>
        </section>
  ...
  function DeskBoard({ awaiting, busState, onAccept, onToast, stage, onResume, showFamilyA }) {
    const liveTraveler = busState && busState.feed ? busState.feed.filter((f) => f.who === "traveler").slice(-1)[0] : null;
    const [view, setView] = React.useState("board");

    const kpis = [
      { lbl: "Commission · this month", val: "£4,120", sub: "+8.2% vs last week", up: true, spark: [610, 580, 720, 690, 760, 820, 790, 880, 940, 905, 1030] },
      { lbl: "On hold", val: "2", sub: "1 expires today 18:00", tone: "warn" },
      { lbl: "Awaiting customer", val: "1", sub: "Traveler F · opened 3×" },
      { lbl: "Self-served in portal", val: "78%", sub: "target 85%", bar: 78 },
    ];

    /* the Family A card moves across the board as the live work package progresses */
    const cs = (() => {
      const s = stage || "browse";
      if (s === "browse") return { col: "new", tag: "24h hold", tagk: "ai", src: "via Travel Concierge · Consumer Travel Portal", trip: "London · Bath · Cornwall", val: "≤ CA$11,000", act: "accept" };
      if (["parsing", "blockers"].includes(s)) return { col: "flight", tag: "Intake", tagk: "info", src: "capturing the brief", trip: "London · Bath · Cornwall", val: "≤ CA$11,000", act: "resume" };
      ...
      if (s === "proposal") return { col: "flight", tag: "Proposal drafted", tagk: "live", src: "proposal V1 · ready to send", trip: "London · Bath · Cornwall", val: "£7,365", act: "resume" };
      if (["held", "repricing"].includes(s)) return { col: "await", tag: "Awaiting reply", tagk: "ok", src: "proposal sent · hold AT-88421", trip: "London · Bath · Cornwall", val: "£7,365", act: "resume" };
      if (s === "v2") return { col: "await", tag: "Change in progress", tagk: "ok", src: "servicing a change · hold AT-88421", trip: "London · Bath · Cornwall", val: "£7,275", act: "resume" };
      if (s === "completed") return { col: "booked", tag: "Booked", tagk: "okfill", src: "trip confirmed · V3", trip: "London · Bath · Cornwall", val: "£7,185", act: "open" };
      return { col: "new", tag: "24h hold", tagk: "ai", src: "via Travel Concierge · Consumer Travel Portal", trip: "London · Bath · Cornwall", val: "≤ CA$11,000", act: "accept" };
    })();
    const familyA = { id: "travelerA", who: "Traveler Family A", cat: "United Kingdom · Family Travel", trip: cs.trip, dates: "12–22 Aug", val: cs.val, src: cs.src, srcIcon: "chat-bot", tag: cs.tag, tagk: cs.tagk, primary: cs.col !== "booked", cday: 12, cnote: "departs", act: cs.act };
    const inject = (key, arr) => (showFamilyA !== false && key === cs.col) ? [familyA, ...arr] : arr;

    const cols = [
      { key: "new", label: "Incoming", dot: "new", clients: inject("new", [
        { id: "travelerB", who: "Traveler Couple B", cat: "Japan · Signature Travel", trip: "Ryokan · omakase · 10 nights", dates: "October", val: "no groups", src: "walk-in · noted by the travel advisor", srcIcon: "group", tag: "New", tagk: "new" },
      ]) },
      { key: "flight", label: "In progress", dot: "live", clients: inject("flight", [
        { id: "travelerD", who: "Traveler Family D", cat: "Maldives · overwater", trip: "Proposal V1 on hold", dates: "AT-88102", val: "£5,940", src: "expires today 18:00", srcIcon: "warning-alt", tag: "Expiring", tagk: "warn", urgent: true, cday: 7, cnote: "hold ends" },
        { id: "travelerE", who: "Traveler E", cat: "Tenerife", trip: "Proposal V3 on hold", dates: "AT-88097", val: "£2,180", src: "expires tomorrow 11:30", srcIcon: "dashboard", tag: "On hold", tagk: "info", cday: 8, cnote: "hold ends" },
        { id: "travelerG", who: "Traveler Group G", cat: "Scottish Highlands · ski", trip: "Quote draft · 12 pax", dates: "−£480 o/n", val: "12 pax", src: "ready to re-share", srcIcon: "analytics", tag: "Draft", tagk: "info" },
      ]) },
      { key: "await", label: "Awaiting reply", dot: "wait", clients: inject("await", [
        { id: "travelerF", who: "Traveler F", cat: "Santorini · Signature Travel", trip: "Proposal V2 shared", dates: "opened 3×", val: "£3,120", src: "follow-up drafted", srcIcon: "recommend", tag: "Sent", tagk: "ok" },
      ]) },
      { key: "booked", label: "Booked", dot: "ok", clients: inject("booked", [
        { id: "travelerC", who: "Traveler Family C", cat: "Devon · Value", trip: "Confirmed · PNR issued", dates: "2–9 Aug", val: "£2,480", src: "booked Tuesday", srcIcon: "checkmark-filled", tag: "Booked", tagk: "okfill", cday: 2, cnote: "departs" },
      ]) },
    ];
    ...
    return (
      <section className="sf-board" data-screen-label="Agent desk">
        <div className="bd-toolbar">
          <div className="bd-title"><h1>Your desk</h1><span className="bd-sub">the travel advisor · Partner Travel Agency — today</span></div>
          ...
          <div className="bd-actions">
            ...
            <button className="primary" onClick={() => onToast("New inquiries arrive from Travel Concierge, email or walk-in.")}>+ Add inquiry</button>
          </div>
        </div>
        <JourneyTakeoff awaiting={awaiting} stage={stage} />
        ...
        <div className="bd-trends" data-screen-label="Trends to pitch">
          ...
          {[
            { n: "United Kingdom · family AI", tone: "#0F62FE" },
            { n: "Maldives · overwater −12%", tone: "#7A2048" },
            { n: "Tenerife · winter sun", tone: "#1F8A5B" },
            { n: "Scottish Highlands · ski week", tone: "#29707A" },
            { n: "Santorini · adults-only", tone: "#8a6116" },
            { n: "Japan · Signature Journey", tone: "#FF462D" },
            { n: "Devon · last-minute", tone: "#0F62FE" },
          ].map((t) => (
            <button className="bd-trend" key={t.n} style={{ "--tc": t.tone }} onClick={() => onToast(`${t.n} — open the storefront to quote this for a client.`)}><span className="d"></span>{t.n}</button>
          ))}
        </div>
          ...
          {cols.map((col) => (
            <div className={`bd-col ${col.key}`} key={col.key}>
              ...
              <div className="bd-col-body">
                {col.clients.map((c) => {
                  const isFamilyA = c.id === "travelerA";
                  const waiting = isFamilyA && awaiting;
                  const click = isFamilyA ? (waiting ? undefined : (c.act === "accept" ? onAccept : onResume)) : () => onToast(`${c.who} — open in the full workbench to continue.`);
                  return (
                    <article className={`bd-card ${c.primary ? "primary" : ""} ${c.urgent ? "urgent" : ""}`} key={c.id}
                      onClick={click} data-screen-label={isFamilyA ? "Incoming handoff" : null}>
                      ...
                      <div className="bd-card-foot">
                        <span className="bd-log">{c.srcIcon === "chat-bot" ? <SfInf size={12} /> : <Ki name={c.srcIcon} size={12} />} {waiting ? "Live on Consumer Travel Portal" : c.src}</span>
                        {isFamilyA
            ...
            {allClients.map((c) => {
              ...
              return (
                <button className={`bd-list-row ${c.primary ? "primary" : ""} ${c.urgent ? "urgent" : ""}`} key={c.id} onClick={click}>
                  ...
                  <span className="ac">{isFamilyA ? (waiting ? "Live" : (c.act === "accept" ? "Accept →" : c.act === "open" ? "Open →" : "Resume →")) : "Open →"}</span>
                </button>
              );
            })}
          </div>
        ...
        {view === "calendar" && (() => {
          const first = new Date(2026, 7, 1).getDay();
          const evs = {}; allClients.forEach((c) => { if (c.cday) (evs[c.cday] = evs[c.cday] || []).push(c); });
          const undated = allClients.filter((c) => !c.cday);
          const WD = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
          const shortName = (n) => n.replace(/^(The |Family|Group|Couple|Sig\.ra|Sig\.) /, "");
          return (
            <div className="bd-cal" data-screen-label="Desk calendar view">
                ...
                {Array.from({ length: 31 }, (_, i) => i + 1).map((d) => (
                  <div className={`bd-cal-cell ${d === 12 ? "today" : ""}`} key={d}>
                    ...
                    {(evs[d] || []).map((c) => {
                      const isFamilyA = c.id === "travelerA";
                      const waiting = isFamilyA && awaiting;
                      const click = isFamilyA ? (waiting ? undefined : (c.act === "accept" ? onAccept : onResume)) : () => onToast(`${c.who} — open in the full workbench.`);
              ...
              {undated.length > 0 && (
                <div className="bd-cal-later">
                  ...
                  {undated.map((c) => <button key={c.id} className="bd-cal-chip" onClick={() => onToast(`${c.who} — ${c.trip}.`)}><span className={`d ${c.dot}`} /> {shortName(c.who)} · {c.cat}</button>)}
                </div>
              )}
            </div>
          );
  ...
  Object.assign(window, { SfProductCard: ProductCard, SfHero: Hero, SfDeskStrip: DeskStrip, SfBrandBand: BrandBand, SfShelfRow: ShelfRow, SfChipsBar: ChipsBar, SfSoldOutBanner: SoldOutBanner, SfCompareTray: CompareTray, SfCompareOverlay: CompareOverlay, SfDeskBoard: DeskBoard });
})();
