/* ============================================================
   ...
   ============================================================ */
(function () {
  const { fmtGBP, pulse, priceCalendar, fitMatrix, milestones, products, desk } = window.SF;
  ...
  function CommissionKpi() {
    ...
    return (
      <div className="sf-pulse-card sf-agent-only">
        ...
        <div className="row">
          <span className="big num">{fmtGBP(c.mtd)}</span>
  ...
  function DeflectionGauge() {
    ...
    return (
      <div className="sf-pulse-card">
        <div className="lab">Self-service rate · Travel Orchestrator</div>
        <div className="row">
  ...
  function BrandMixDonut() {
    ...
    return (
      <div className="sf-pulse-card">
        ...
        <div className="csrc"><span className="d" /> Booking platform · Partner Travel Agency · 12-week window</div>
      </div>
    );
  }
  ...
  function FitMatrix({ onSource }) {
    return (
      <section className="sf-matrixcard" data-screen-label="Constraint-fit matrix">
        <div className="mh">
          <div>
            <div className="ct">Constraint fit · Family A brief</div>
      ...
      <ChartCard
        ...
        build={() => ({
          ...
          options: {
            ...
            plugins: { legend: { display: false }, tooltip: { callbacks: { label: (x) => x.datasetIndex === 0 ? "Budget cap £3,500" : " £" + x.raw.toLocaleString("en-GB") + (x.dataIndex === pc.requested ? " · requested window" : "") } } },
            scales: {
      ...
      <ChartCard
        ...
        build={() => ({
          ...
          options: {
            ...
            plugins: { legend: { display: false }, tooltip: { callbacks: { label: (x) => x.dataIndex === 1 ? ` ${up ? "+" : "−"}£${Math.abs(delta)}` : " £" + x.raw[1].toLocaleString("en-GB") } } },
            scales: { x: { ...grid, grid: { display: false } }, y: { ...grid, min: lo, max: hi, ticks: { callback: (v) => "£" + v.toLocaleString("en-GB") } } },
          },
        })}
      />
    );
      ...
      <ChartCard
        ...
        build={() => ({
          ...
          data: {
            ...
            datasets: [
              { label: "With Travel Orchestrator", data: milestones.assistant, backgroundColor: "#29707A", borderRadius: 2, barThickness: 13 },
