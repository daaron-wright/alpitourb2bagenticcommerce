/* ============================================================
   EasyBook Next — B2B Storefront · Travel Orchestrator surfaces
   Brand-pick tiles (in chat) · workshop sticky receipts ·
   live-updates feed · next-best-action card with package
   includes · rendered proposal.md preview · Signature Travel panel.
   ============================================================ */
(function () {
  const { fmtGBP, orchestrator } = window.SF;

  /* ---------- official 7-step journey strip ---------- */
  function JourneyStrip({ active }) {
    const steps = orchestrator.useCase.steps;
  ...
  /* ---------- workshop sticky receipt — removed (no "Workshop note" surface) ---------- */
  function Sticky() {
    return null;
  }

  /* ---------- in-chat incoming requests — no brand pick; semantics decide ---------- */
  function RequestPick({ chosen, onPick }) {
    /* platform mode: the Family A card reflects the live Q&A from Consumer Travel Portal */
    const brief = window.PlatformBus && window.PlatformBus.state.brief;
    return (
      <div className="sf-rq" data-screen-label="Incoming requests">
        {orchestrator.requests.map((r) => {
          const isB = r.id === "travelerA";
          const signals = isB && brief ? brief.answers.map((a) => a.v) : r.signals;
          const meta = isB && brief ? "via Travel Concierge · cluster confirmed during inquiry · 24h hold" : r.meta;
          return (
            <button key={r.id} className={`sf-rq-card ${chosen === r.id ? "on" : ""} ${chosen && chosen !== r.id ? "dim" : ""}`} onClick={() => onPick(r.id)} disabled={!!chosen}>
              <span className="rh"><Ki name="group" size={12} /> <b>{r.who}</b><span className="m">{meta}</span></span>
              <span className="rw">“{r.words}”</span>
              <span className="rs">{signals.map((s) => <i key={s}>{s}</i>)}</span>
              {chosen === r.id ? <span className="picked"><Ki name="checkmark-filled" size={11} /> Working this request</span> : <span className="cta">Work this request →</span>}
            </button>
          );
        })}
      </div>
  ...
  function ClusterMatch({ c }) {
    const hit = orchestrator.clusters.find((x) => x.id === c.cluster);
    return (
      <div className="sf-cm" data-screen-label="Cluster match">
        ...
        <div className="grid8">
          {orchestrator.clusters.map((x) => (
            <span key={x.id} className={`cell ${x.id === c.cluster ? "hit" : ""}`} style={x.id === c.cluster ? { "--ct": x.tone } : null} title={x.name}>
              <i className="num">{x.n}</i>{x.id === c.cluster && <b>{x.name}</b>}
            </span>
          ))}
        </div>
        <div className="cr">Cluster <b className="num">{hit.n}/8 · {hit.name}</b> → brand <b style={{ color: hit.tone }}>{hit.brandName}</b> — same cluster that personalizes consumer travel portal: <i>{hit.push.toLowerCase()}</i>.</div>
      </div>
    );
  }
    ...
    return (
      <section className="sf-cp" data-screen-label="B2C personalization program">
        <div className="sf-shelf-h">
          ...
          <span className="sub">8 clusters push personalization on consumer travel portal — the same semantics curate the brand here, automatically</span>
        </div>
        <div className="cpgrid">
          {orchestrator.clusters.map((c) => (
            <div className="cpcard" key={c.id} style={{ "--ct": c.tone }}>
  ...
  function MapPanel({ mode, onOpenPin }) {
    const ref = React.useRef(null);
    const mapRef = React.useRef(null);
    const markersRef = React.useRef({});
    const d = orchestrator.maps[mode];
    const products = window.SF.products;

    // the preferences handed over from the end customer — toggle to explore the map
    const PREFS = [
      { id: "baby", label: "Family rooms", test: (p) => p.amenities.includes("Family rooms") || p.amenities.includes("Family suite") },
      { id: "kids", label: "Short transfers", test: (p) => p.amenities.includes("Short transfers") || p.amenities.includes("Private transfers") },
      { id: "budget", label: "≤ £7,400", test: (p) => p.price <= 7400 },
      { id: "avail", label: "Available", test: (p) => p.allotment && p.allotment.status !== "sold_out" },
    ];
    const [filters, setFilters] = React.useState({ baby: true, kids: true });
    const pinCls = (p) => p.kind === "sold" ? "sold" : p.kind === "best" ? "best" : p.kind === "airport" ? "airport" : p.kind === "stop" ? "stop" : "price";
    const matchPin = (p) => {
    ...
    React.useEffect(() => {
      if (!window.L || !ref.current || !d) return;
      const L = window.L;
      ...
      d.pins.forEach((p) => {
        const icon = L.divIcon({ className: "", html: `<span class="sf-mappin ${pinCls(p)}">${p.label}</span>`, iconSize: null, iconAnchor: p.kind === "stop" ? [13, 13] : [28, 30] });
        const mk = L.marker(p.ll, { icon }).addTo(map);
        mk.bindPopup(`<b>${p.name}</b><br/><span style="font-size:11px;color:#555">${p.meta}</span>`, { closeButton: false, offset: [0, -18] });
    ...
    // re-style pins as the agent adjusts the customer-preference filters
    React.useEffect(() => {
      if (!d) return;
      d.pins.forEach((p) => {
    ...
    const placePins = d.pins.filter((p) => p.kind !== "airport" && p.kind !== "stop");
            ...
            {d.pins.map((p) => {
              ...
              return (
                <button key={p.id} className={`mprow ${place && !ok ? "dim" : ""}`} onClick={() => goTo(p)}>
                  ...
                  {mode === "family" && place && ok && <span className="mtok"><Ki name="checkmark-filled" size={13} /></span>}
                </button>
              );
            })}
          </aside>
  ...
  function ChoicesTimeline({ choices, timeline }) {
  ...
  /* ---------- in-chat brand selection (legacy — kept for reference) ---------- */
  function BrandPick({ chosen, onPick }) {
    return (
      <div className="sf-bp" data-screen-label="Brand selection">
        {orchestrator.brandPick.map((b) => (
          <button key={b.id} className={`sf-bp-tile ${chosen === b.id ? "on" : ""} ${chosen && chosen !== b.id ? "dim" : ""}`}
            style={{ "--bt": b.tone }} onClick={() => onPick(b.id)} disabled={!!chosen}>
            ...
            <span className="bl">{b.bullets.map((x) => <span key={x}><Ki name="checkmark-filled" size={10} /> {x}</span>)}</span>
  ...
  function LiveFeed({ items, sticky }) {
  ...
  function NextBestAction({ n, onCta }) {
    return (
  ...
  function MdPreview({ brand, price, holdRef, onClose, onSend }) {
    const signatureMode = brand === "signature";
    const j = orchestrator.japan;
    const file = signatureMode ? "proposal-travelerB.md" : "proposal-familyA.md";
    return (
      <div className="sf-overlay" onClick={onClose}>
        <div className="sf-md" onClick={(e) => e.stopPropagation()} data-screen-label="Markdown proposal preview">
          <div className="mh">
            ...
            <div><div className="fn num">{file}</div><div className="fs">Written by {signatureMode ? "Travel Orchestrator" : "Travel Concierge"} · markdown · rendered preview</div></div>
          ...
          <div className="mb">
            ...
            {signatureMode ? (
              <div className="doc">
                ...
                <p>{j.logistics}</p>
                <h2>Price</h2>
                <p><b className="num">{fmtGBP(price)}</b> per couple, all inclusive as itemized. The fare is watched in real time: if the flight drops, the quote drops — it already happened once while we were preparing this proposal (−£120).</p>
                <h2>Terms</h2>
                <p>Free 72-hour option · balance due 30 days before departure · cancellation insurance included.</p>
                <p className="sig">Partner Travel Agency · the travel advisor — with Signature Travel, signature journeys.</p>
              </div>
            ) : (
              <div className="doc">
                <h1>10 Days in the United Kingdom — The Traveler Family A</h1>
                <p className="lead">London · Bath · Cornwall · 10 days · 2 adults + 1 child (age 2)</p>
                <h2>Why this trip</h2>
                <p>The St Ives sea-view hotel you first picked is sold out on your dates — verified against live inventory, not the catalogue. This itinerary keeps everything that mattered: <b>family rooms, air-conditioning and short transfers</b>, the same three cities, and a slow, food-and-history pace — £250 under your budget.</p>
                <h2>The itinerary</h2>
                <p className="day"><b>London · days 1–4</b> — central family suite, slow mornings, a Thames boat afternoon and the British Museum with a family guide</p>
                <p className="day"><b>Bath · days 5–7</b> — family rooms near the Roman Baths, a market food walk and the Royal Crescent before the crowds</p>
                <p className="day"><b>Cornwall · days 8–10</b> — family rooms above the sea, a gentle boat afternoon and a relaxed seaside lunch</p>
                <h2>Included</h2>
                ...
                <h2>Price</h2>
                <p><b className="num">{fmtGBP(price)}</b> per family, all-in (≈ CA$10,900){holdRef ? <> · hold confirmed <b className="num">{holdRef}</b> until tomorrow 6:00 PM</> : null}.</p>
                <h2>Terms</h2>
                <p>Free date change until confirmation · balance due 30 days before departure · live disruption support throughout the trip.</p>
                <p className="sig">Partner Travel Agency · the travel advisor — with Travel Group, United Kingdom made easy.</p>
              </div>
            )}
            </div>
              ...
              <div className="ms-flow">
                {[
                  ...
                  <div className="ms-chan" key={c.n}>
                    <span className="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{c.ic}</svg></span>
  ...
  /* ---------- Signature Travel curated panel (center, selling mode) ---------- */
  function SignatureTravelPanel({ price, repriced, onGenMd, onSource }) {
    const j = orchestrator.japan;
    return (
      <section className="sf-signature" data-screen-label="Signature Travel curated itinerary">
        <div className="th" style={{ "--bt": "#8a6116" }}>
          <span className="sw">T</span>
          <div>
            <div className="eyebrow">Signature Travel · Signature itineraries — curated by Travel Orchestrator</div>
            <h2>{j.title}</h2>
        ...
        <div className="tgrid">
          ...
          <aside className="side">
            <div className="price">
              ...
              <div className="pv num">{fmtGBP(price)}</div>
              {repriced && <div className="pd"><Ki name="analytics" size={11} /> −£120 — live price watch repriced the Airline Partner fare</div>}
              <div className="pc sf-agent-only num">your commission {fmtGBP(Math.round(price * 0.13))} · 13%</div>
            </div>
            <div className="incl">
              <div className="il">The package includes</div>
              {orchestrator.includes.signature.map((x) => (
  ...
  /* ---------- journey board — roomy full-width version of the 7-step use case ---------- */
  function JourneyBoard({ jstep, confidence }) {
    return (
      <section className="sf-jboard" data-screen-label="Journey overview">
        ...
        <div className="jb-steps">
          {orchestrator.useCase.steps.map((s) => (
  ...
  Object.assign(window, { SfSticky: Sticky, SfBrandPick: BrandPick, SfLiveFeed: LiveFeed, SfNextBestAction: NextBestAction, SfMdPreview: MdPreview, SfSignatureTravelPanel: SignatureTravelPanel, SfRequestPick: RequestPick, SfClusterMatch: ClusterMatch, SfClusterProgram: ClusterProgram, SfMapPanel: MapPanel, SfChoicesTimeline: ChoicesTimeline, SfJourneyStrip: JourneyStrip, SfJourneyBoard: JourneyBoard });
})();
