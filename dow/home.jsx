/* ============================================================
   Home — Dow.com marketing homepage with a persona-aware
   welcome-back agent strip for the signed-in customer.
   ============================================================ */
(function () {
  const UI = window.UI;
  const { Icon, Stroke } = UI;

  function Home({ onNav, onSearch, onAskAgent, persona }) {
    const D = window.DX;
    const t = D.typeById(persona) || D.customerTypes[0];
    const sugs = D.suggestions[persona] || D.suggestions.explorer;
    return (
      <div className="dx-scroll fade-in">
        {/* hero */}
        <section className="dx-hero">
          <image-slot id="dx-hero-bg" shape="rect" placeholder="Drop a hero image (optional)"></image-slot>
          ...
          <div className="dx-welcome">
            <div className="dx-welcome-in">
              <div className="dx-welcome-head">
                ...
                <div>
                  <div className="wt">Welcome back, {D.account.first}.</div>
                  <div className="ws">{D.account.role} · {D.account.company} — one Dow polyolefin-elastomer platform across your footwear, packaging and mobility lines. I follow every application and adapt — discover, sample, troubleshoot, scale.</div>
                </div>
              ...
              <div className="dx-sugs">
                {sugs.concat(otherJobTiles(persona)).slice(0, 3).map((s) =>
              ...
              <div className="dx-jobsrow">
                ...
                {D.customerTypes.map((ct) =>
                <button key={ct.id} className={`dx-jobpill ${persona === ct.id ? "on" : ""}`} onClick={() => { const sg = (D.suggestions[ct.id] || [{}])[0]; onAskAgent({ intent: ct.mode, text: sg.text, code: sg.code, compose: sg.compose, placeholder: sg.placeholder, fresh: true }); }} title={ct.job}>
        ...
        <section className="dx-wrap dx-section">
          ...
          <div className="dx-indgrid">
            ...
            <div className="dx-ind" key={ind.id} style={{ "--ind-a": ind.accent }} onClick={() => onSearch("seal resin for flexible food packaging below 120°C")}>
  ...
  // fill the welcome row to 3 tiles with adjacent jobs
  function otherJobTiles(persona) {
    const D = window.DX;
    const order = ["explorer", "builder", "buyer", "operator", "evaluator", "approver", "sponsor"].filter((p) => p !== persona);
    return order.map((p) => (D.suggestions[p] || [])[0]).filter(Boolean);
  }
