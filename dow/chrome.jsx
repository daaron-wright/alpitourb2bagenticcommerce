/* ============================================================
   Dow.com chrome — utility bar, masthead (+ mega-nav), footer,
   floating ChemAssist launcher.
   ============================================================ */
(function () {
  const UI = window.UI;
  const { Icon, Stroke } = UI;

  function UtilityBar({ onNav, onGoOperator, onOpenAlert }) {
    const D = window.DX;
    return (
  ...
  function Masthead({ onNav, onSearch, onAskAgent, cartCount }) {
    const D = window.DX;
    const [q, setQ] = React.useState("");
    const [mega, setMega] = React.useState(null);
    const ref = React.useRef(null);
    React.useEffect(() => {
      function onDoc(e) { if (ref.current && !ref.current.contains(e.target)) setMega(null); }
              ...
              {mega === "Products" ? (
                <React.Fragment>
                  <div className="dx-mega-col">
                    ...
                    {D.products.map(p => <a key={p.id} onClick={() => { onNav({ name: "product", id: p.id }); setMega(null); }}>{p.name}</a>)}
                  </div>
                  <div className="dx-mega-col">
                ...
                D.industries.slice(0, 3).map((ind, i) => (
