/* ============================================================
   ...
   ============================================================ */
(function () {
  const { fmtEUR, products, availability, addons, brands } = window.SF;
          ...
          <div className="sf-drawer-body">
            ...
            <div className="sf-sec">
              ...
              <div className="sf-rate">
                ...
                {addons.map((a) => selAddons.includes(a.id) && <div className="row" key={a.id}><span className="l">{a.name}</span><b className="num">{fmtEUR(a.price)}</b></div>)}
    ...
    return (
      <section className="sf-pkg" data-screen-label="Proposal panel">
        ...
        <div className="sf-lines">
          ...
          {adds.map((a) => <div className="sf-line" key={a.id}><Ki name="recommend" size={14} /> <span>{a.name} <span className="m">{a.why}</span></span><span className="pr num">{fmtEUR(a.price)}</span></div>)}
        ...
        {v.delta && (
          <div className="sf-diff" style={{ marginTop: 11 }}>
            <b>{v.versionId} vs {v.delta.vs}:</b> {v.delta.amount < 0 ? "−" : "+"}{fmtEUR(Math.abs(v.delta.amount))} — {v.delta.reason}. Hotel, room type and amenities are identical. <SfSrcChip ids={["pol1"]} onSource={onSource} />
          </div>
        )}
      </section>
    );
