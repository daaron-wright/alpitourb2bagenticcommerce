/* ============================================================
   ...
   ============================================================ */
(function () {
  const { fmtEUR, brands, products, SHELVES, BRIEF, chips, desk } = window.SF;
  ...
  /* ---------- customer reference — collapsible, human-centered ----------
     The end customer (the Bianchi family) lives here: who they are, their
  ...
  function ChipsBar({ blockers, onFix, stage }) {
    const rq = ((window.SF.tredi && window.SF.tredi.requests) || []).find((r) => r.id === "bianchi") || {};
    ...
    return (
      <div className={`sf-cust ${expanded ? "open" : ""}`} data-screen-label="Customer brief">
        <button className="sf-cust-bar" onClick={() => setOpen((o) => !o)} aria-expanded={expanded}>
          ...
          <span className="src"><SfInf size={11} /> via AlpiConcierge · Alpitour.it</span>
  ...
  function JourneyTakeoff({ awaiting }) {
    ...
    const active = awaiting ? 0 : 1; // inquiry received once it lands on the desk
  ...
  function DeskOverview({ onToast }) {
    ...
    const dests = [
      { id: "bianchi", ll: [41.90, 12.50], who: "Carter", place: "Italy · Rome", tone: "#0F62FE" },
      { id: "ferri", ll: [3.2, 73.0], who: "Ferri", place: "Maldives", tone: "#7A2048" },
      { id: "romano", ll: [28.29, -16.62], who: "Romano", place: "Tenerife", tone: "#1F8A5B" },
      { id: "greco", ll: [45.93, 7.63], who: "Greco", place: "Cervinia", tone: "#29707A" },
      { id: "conti", ll: [36.39, 25.46], who: "Conti", place: "Santorini", tone: "#8a6116" },
      { id: "ferrante", ll: [35.68, 139.77], who: "Ferrante", place: "Japan", tone: "#FF462D" },
      { id: "deluca", ll: [39.2, 9.1], who: "De Luca", place: "Sardinia", tone: "#0F62FE" },
    ];
    React.useEffect(() => {
    ...
    const evd = { 2: { c: "#1F8A5B", t: "De Luca · Sardinia departs" }, 7: { c: "#E6A100", t: "Ferri · Maldives hold ends" }, 8: { c: "#0F62FE", t: "Romano · Tenerife hold ends" }, 12: { c: "#FF462D", t: "Carter · Italy departs" } };
  ...
  function DeskBoard({ awaiting, busState, onAccept, onToast, stage, onResume, showCarter }) {
    ...
    /* the Carter card moves across the board as the live work package progresses */
    const cs = (() => {
    ...
    const carter = { id: "bianchi", who: "The Carter family", cat: "Italy · Bravo", trip: cs.trip, dates: "12–22 Aug", val: cs.val, src: cs.src, srcIcon: "chat-bot", tag: cs.tag, tagk: cs.tagk, primary: cs.col !== "booked", cday: 12, cnote: "departs", act: cs.act };
    const inject = (key, arr) => (showCarter !== false && key === cs.col) ? [carter, ...arr] : arr;
    ...
    return (
      <section className="sf-board" data-screen-label="Agent desk">
                ...
                {col.clients.map((c) => {
                  ...
                  return (
                    <article className={`bd-card ${c.primary ? "primary" : ""} ${c.urgent ? "urgent" : ""}`} key={c.id}
                      onClick={click} data-screen-label={isBianchi ? "Incoming handoff" : null}>
                      ...
                      <div className="bd-card-foot">
                        <span className="bd-log">{c.srcIcon === "chat-bot" ? <SfInf size={12} /> : <Ki name={c.srcIcon} size={12} />} {waiting ? "Live on Alpitour.it" : c.src}</span>
                        {isBianchi
        ...
        {view === "calendar" && (() => {
          ...
          const shortName = (n) => n.replace(/^(The |Famiglia|Gruppo|Coppia|Sig\.ra|Sig\.) /, "");
          return (
