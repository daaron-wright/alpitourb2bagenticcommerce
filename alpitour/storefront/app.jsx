...
(function () {
  ...
  const { fmtEUR, products, SHELVES, STAGE_LABEL, blockers: BLOCKERS, tredi } = window.SF;
  ...
  function App() {
    ...
    function startLive(id) {
      ...
      const evs = tredi.liveEvents[id] || tredi.liveEvents.bravo;
      const fire = () => {
        ...
        if (ev.priceDelta) { setTuriPrice((p) => p + ev.priceDelta); setTuriRepriced(true); showDelta(ev.delta); }
    ...
    function workRequest(id) {
      ...
      const rq = tredi.requests.find((r) => r.id === id);
      const cl = tredi.clusters.find((c) => c.id === rq.cluster);
      setRequest(id);
      setBrand(cl.brand);
      startLive(cl.brand);
    ...
    function sendMd() {
      ...
      if (!turi) { PB && PB.emit("proposal", { file: "proposal-carter.md" }); pbFeed("Proposal sent — it's on your trip page"); }
      pushHandoff({
    ...
    function approveV3() {
      ...
      orchestrate({
        ...
        then: () => {
          ...
          PB && PB.emit("v2", { total: 7185, window: "15–25 August · Amalfi by ferry" });
      ...
      <div className={`sf-app ${t.density === "dense" ? "dense" : ""} ${t.customerSafe ? "safe" : ""} ${t.showPhotos ? "" : "noimg"}`}>
        ...
        <div className={`sf-main ${railOpen ? "" : "rail-closed"}`}>
          <div className="sf-shelf-col" ref={shelfRef} data-screen-label={selling ? (stage === "turisanda" ? "Turisanda curated" : "Selling shelf") : "Storefront"}>
            {selling && !homeView && stage !== "turisanda" && <SfChipsBar blockers={blk} onFix={fixBlocker} stage={stage} />}
            <div className="sf-shelf-in">
              {(!selling || homeView) && (
                <SfDeskBoard awaiting={awaitingInquiry} busState={busState} onAccept={acceptHandoff} onToast={showToast} stage={stage} onResume={() => { setHomeView(false); scrollTop(); }} showCarter={inquiryShown || stage !== "browse"} />
              )}

              {!homeView && stage === "turisanda" && (
                <>
                  <SfTurisandaPanel price={turiPrice} repriced={turiRepriced} onGenMd={() => onQuick({ id: "genmd" })} onSource={onSource} />
                  <SfJourneyGraph mode="turisanda" turiPrice={turiPrice} turiRepriced={turiRepriced} mdGenerated={mdGen} mdSent={mdSent} />
                  <SfMapPanel mode="turisanda" />
                </>
              )}
              ...
              {!homeView && showRanked && (
                <>
                  <SfSoldOutBanner />
                  <SfMapPanel mode="bravo" onOpenPin={(id) => { if (products[id]) setDrawerId(id); }} />
        ...
        {mdOpen && <SfMdPreview brand={brand || "bravo"} price={brand === "turisanda" ? turiPrice : ((versions.find((v) => v.status === "held") || versions[0] || {}).total || 3465)} holdRef={holdRef} onClose={() => setMdOpen(false)} onSend={() => onQuick({ id: "sendmd" })} />}
