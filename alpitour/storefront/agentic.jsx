...
(function () {
    ...
    return (
      <div className={`sf-asb ${state}`} data-screen-label="Agent status bar">
        ...
        <span className="controls">
          {state === "thinking" && <button className="b" onClick={() => onToast("Run paused — resume from the status bar. (Simulated in this prototype.)")}>Pause</button>}
  ...
  function Args({ args }) {
    const lines = Object.entries(args).map(([k, v]) => (
      <div key={k}><span className="k">{k}</span>: {typeof v === "number" ? <span className="n">{v}</span> : <span className="s">"{v}"</span>}</div>
    ));
    return <div className="sf-tcc-args">{lines}</div>;
  }

  function ToolCallCard({ t, onApprove, onReject }) {
    const [open, setOpen] = React.useState(t.status === "pending");
    const statusLabel = t.status === "running" ? "Executing" : t.status === "ok" ? "Completed" : t.status === "err" ? "Failed" : "Awaiting approval";
    return (
      <div className={`sf-tcc ${t.status}`} data-screen-label={`Tool call ${t.tool}`}>
        <div className="head" role="button" tabIndex={0} onClick={() => setOpen(!open)} onKeyDown={(e) => e.key === "Enter" && setOpen(!open)}>
      ...
      <div className={`sf-handoff ${h.dir}`} data-screen-label="Agent handoff">
        ...
        {h.payload && (
          <div className="payload">
            {h.payload.map((p) => <span key={p}><Ki name="checkmark-filled" size={10} /> {p}</span>)}
          </div>
        )}
  ...
  function OvernightPanel({ onDelta, onToast }) {
    const data = window.SF.overnight;
    const [state, setState] = React.useState({}); // id -> 'done' | 'dismissed'
    const open = data.items.filter((i) => !state[i.id]);
    ...
    return (
      <section className="sf-ov" data-screen-label="Overnight agent run">
        ...
        {open.length === 0 ? (
          <div className="sf-ov-empty"><Ki name="checkmark-filled" size={13} /> Desk clear — every overnight action handled. The agent logged your choices to keep learning your defaults.</div>
        ) : (
          <div className="sf-ov-rows">
            {open.map((it) => (
              <div key={it.id} className="sf-ov-row">
                ...
                <div className="acts">
                  <button className="sf-btn sm primary" onClick={() => { setState((s) => ({ ...s, [it.id]: "done" })); onDelta(it.delta); }}>{it.approveLabel}</button>
                  <button className="sf-btn sm" onClick={() => { setState((s) => ({ ...s, [it.id]: "dismissed" })); onToast("Dismissed — logged as a teaching signal. The agent will deprioritise similar suggestions."); }}>Dismiss</button>
                </div>
              </div>
            ))}
          </div>
  ...
  Object.assign(window, { SfAgentStatusBar: AgentStatusBar, SfToolCallCard: ToolCallCard, SfAiGlyph: AiGlyph, SfHandoffCard: HandoffCard, SfOvernightPanel: OvernightPanel });
})();
