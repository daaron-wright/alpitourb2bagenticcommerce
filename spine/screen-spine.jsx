...
(function () {
    ...
    return (
      <div style={{ marginBottom: 22 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.35fr", gap: 14, marginTop: 14, alignItems: "stretch" }}>
          <ChartCard title="What the agents watch" sub="Signal volume by source feed · one week. The Anomaly agent reads ERP, plant MES, logistics TMS and market feeds together.">
          ...
          <ChartCard title="Anomaly pressure by feed × week" sub="Where signals broke planning assumptions · 12-week heatmap. Feedstock and market price run hottest.">
  ...
  function LiveCustomerEvents({ setScreen }) {
    const AG = {
      concierge: { n: "Sales Concierge", c: "#6B36A8", t: "#F0E6FA", icon: "chat-bot" },
      deal: { n: "Account & Deal Mgr", c: "#B45309", t: "#FEF3E2", icon: "network" },
      supply: { n: "Supply & Fulfilment", c: "#0E7490", t: "#E0F2F4", icon: "network" },
      compliance: { n: "Compliance / PAC", c: "#15803D", t: "#E7F6EC", icon: "anomaly" },
      regradar: { n: "RegRadar", c: "#B45309", t: "#FEF3E2", icon: "anomaly" },
    };
    const EVENTS = [
      { evt: "EVT-7762", trace: "KAF-5B70-1180", cust: "Brandt order PO-48261 · plant outage hit Line 2", ts: "live · 18:42",
        steps: [
          ["supply", "Reallocated 250 kg to Tarragona by ATP + contract priority"],
          ["compliance", "PAC check: strategic-grade covenant preserved — allowed"],
          ["concierge", "Notified Brandt: promise held at 15 May"],
        ] },
      { evt: "EVT-7758", trace: "KAF-5B70-1180", cust: "Brandt changed the order — quantity +50 kg", ts: "live · 09:21",
        steps: [
          ["concierge", "Received the change, opened the case"],
          ["deal", "Re-priced & re-ran ATP against account terms"],
          ["supply", "Re-committed delivery — 16 May, 92% confidence"],
        ] },
      { evt: "EVT-7770", trace: "KAF-5B70-1180", cust: "Carrier weather risk on the Brandt shipment", ts: "live · 14:05",
        steps: [
          ["supply", "Digital twin predicted +1 day slip"],
          ["regradar", "No regulatory impact on reroute"],
          ["concierge", "Proactively notified Brandt with a recovery option"],
        ] },
      { evt: "EVT-7741", trace: "KAF-9F2A-4471", cust: "Brandt food-contact case · EU 10/2011 update", ts: "live · 31 May",
        steps: [
          ["regradar", "Detected Annex I SML change, mapped to AFFINITY™ PL 1881G"],
          ["compliance", "PAC: informational — no certification implied; declaration refresh queued"],
          ["concierge", "Surfaced a proactive alert on Brandt's case"],
        ] },
    ];
    ...
    return (
      <div className="lce">
        ...
        <div className="lce-foot"><Icon name="information" size={12} /> The same events Brandt sees in their order experience — one record, both lenses. <a onClick={() => setScreen("flow")}>Open RegRadar ↗</a> · <a onClick={() => setScreen("governance")}>PAC decisions ↗</a></div>
      </div>
    );
  }
      ...
      <div>
        <div className="page-head">
          <div>
            ...
            <div className="ph-sub">A small team of agents sits inside Dow's planning loop, watching every signal and acting only within the bounds Finance, Operations and Compliance have set. KAF runs the agents. PAC holds the rules. Every agent action asks the policy engine before it commits.</div>
          </div>
          <div className="ph-spacer" />
          <Button variant="prim" icon="anomaly" onClick={() => setScreen("flow")}>See it act → supply disruption</Button>
        </div>
        ...
        <div className="spine-eyebrow" style={{ marginTop: 18 }}>
          ...
          <span className="se-r">Compiled from the BRD · {window.MACHINE ? "" : ""}198 policies · 412 tests pass</span>
        </div>
        <div className="pacrules">
          {[
            { id: "OPS-auto-ticket-EU", bound: "Plant SOPs", tone: "ops", policy: "Monitoring may open a ticket and alert the owner automatically — no sign-off needed." },
            { id: "FIN-margin-floor-EU", bound: "Finance", tone: "fin", policy: "Margin-floor breaches are flagged to Finance. An agent never moves price on its own." },
            { id: "PROD-covenant-brandt", bound: "Product", tone: "prod", policy: "A sourcing change must preserve the contracted grade and spec for strategic-tier customers." },
            { id: "PROD-customer-commitment", bound: "Product", tone: "prod", policy: "A customer-visible commitment can't originate from an agent — it routes to Sales." },
            { id: "TRADE-covenant-EU", bound: "Trade compliance", tone: "trade", policy: "A covenant-affecting commit must be confirmed by the planner who owns the account." },
            { id: "PROD-forecast-version", bound: "Product", tone: "prod", policy: "Internal forecast updates are auto-authorized when versioned and audited." },
          ].map(r => (
