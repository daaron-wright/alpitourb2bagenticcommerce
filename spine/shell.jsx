/* ============================================================
   AppShell — Dow × Kyndryl chrome, Operator↔Customer lens
...
(function () {
  ...
  const NAV = {
    ops: [
      ...
      { id: "brd", label: "Business Requirements · Upload", icon: "document-chart" },
      { id: "spine", label: "The spine", icon: "network" },
      { id: "ontology", label: "Group ontology", icon: "network" },
      { id: "flow", label: "RegRadar", icon: "anomaly", badge: "live", badgeTone: "spruce" },
      { group: "Governance" },
      ...
      { id: "o2c", label: "Order-to-Cash scorecard", icon: "analytics", badge: "live", badgeTone: "spruce" },
      { id: "controls", label: "Controls & evidence", icon: "document-chart" },
    ],
    cx: [
      { group: "Your order" },
      { id: "customer", label: "Order status", icon: "group" },
    ],
  };
  ...
  function AppShell({ lens, setLens, screen, setScreen, collapsed, setCollapsed, children }) {
    const D = window.D;
    const nav = NAV[lens];
    // Customer lens renders the full-bleed Dow.com storefront — no Kyndryl chrome.
    if (lens === "cx") return <div className="app-cx">{children}</div>;
    return (
      <div className={`app ${collapsed ? "nav-collapsed" : ""}`}>
        {/* Brand */}
        <div className="brandcell">
          <img className="brand-kyn-logo" src="assets/kyndryl-logo.png" alt="Kyndryl" />
          <span className="brand-div" />
          <img className="brand-dow-logo" src="assets/dow-logo.png" alt="Dow" />
        </div>
