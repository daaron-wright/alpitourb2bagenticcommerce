/* ============================================================
   AppShell — Travel Group chrome, Operator↔Customer lens
...
(function () {
  ...
  function AppShell({ lens, setLens, screen, setScreen, collapsed, setCollapsed, children }) {
    const D = window.D;
    const nav = NAV[lens];
    // Customer lens renders the full-bleed Travel Group front end — no operator chrome.
    if (lens === "cx") return <div className="app-cx">{children}</div>;
    return (
      <div className={`app ${collapsed ? "nav-collapsed" : ""}`}>
        {/* Brand */}
        <div className="brandcell">
          <img className="brand-dow-logo" src="assets/kyndryl-logo.png" alt="Kyndryl" />
        </div>
