...
(function () {
  ...
  function App() {
    ...
    // --- Live FE thread (Travel Group Platform.html) → operator toasts ---
    const lastThreadKey = React.useRef(null);
    React.useEffect(() => {
      if (!window.SpineSync) return;
      const MSG = {
        handoff: "Consumer Travel Portal · live — the Traveler Family A sent their inquiry to Partner Travel Agency.",
        brief: "Consumer Travel Portal · live — Travel Assistant structured the family's brief into requirements.",
        accepted: "EasyBook · live — the travel advisor accepted the Family A thread and is working it.",
        hold: "EasyBook · live — a hold was placed on the Family A booking.",
    ...
    React.useEffect(() => {
      ...
      if (naphtha.secured && !announced.current.secured) {
        ...
        window.SpineSync.announce({ t: "Tour-ops spine · rooms re-secured from the Family TravelClub guaranteed block — the Family A 14-Aug dates hold (TRC-2026-0810)." });
      }
    ...
    const setLens = React.useCallback((l) => {
      if (l === "cx") { window.location.href = "Travel Group Platform.html?lens=cx"; return; }
      setLensRaw(l);
    ...
    React.useEffect(() => {
      if (naphtha.learned && lens === "ops" && !notifiedLearn.current) {
        ...
        toast("Partner Travel Agency sent insight on the resort event — a new policy proposal is in “Learned from you”.", { label: "Review →", onClick: () => setScreen("learning") });
      }
