...
(function () {
  ...
  // ---- The constrained rotation: Airline Partner LHR → SSH ----
  const PLANT = { name: "Airline Partner LHR → SSH", region: "EG", capacity: 1800 };
  const GRADES = [
    { id: "jaz-mirabel", name: "Jaz Mirabel Beach", short: "Jaz Mirabel", app: "Family Travel · family AI", accent: "#1E5BB8", demand: 920, price: 2380, marginPct: 22, dyn: 4, covenant: true, account: "Partner Travel Agency · contracted" },
    { id: "coral-bay", name: "Coral Bay Family Resort", short: "Coral Bay", app: "Family Travel · family AI", accent: "#0F766E", demand: 760, price: 2520, marginPct: 26, dyn: 3, covenant: false, account: "Partner Agency Network agencies" },
    { id: "redsea-diving", name: "Red Sea diving week", short: "Red Sea diving", app: "Travel Group · active", accent: "#2563EB", demand: 700, price: 2180, marginPct: 18, dyn: 2, covenant: false, account: "Online channels" },
  ];
  const GID = {}; GRADES.forEach(g => GID[g.id] = g);
  const TOTAL_DEMAND = GRADES.reduce((a, g) => a + g.demand, 0); // 2380

  const OPTIONS = [
    { id: "A", rec: true, name: "Protect covenant demand", sub: "Cover Partner Travel Agency's contracted Jaz Mirabel seats in full, then highest margin.",
      alloc: { "jaz-mirabel": 920, "coral-bay": 600, "redsea-diving": 280 }, covenant: "held",
      pac: { rule: "PROM-covenant-agency", outcome: "allow", note: "Partner Travel Agency's contracted seats fully covered — covenant held, no human gate." } },
    { id: "B", name: "Maximise margin", sub: "Prioritise the highest-margin packages regardless of contract.",
      ...
      pac: { rule: "PROM-covenant-agency", outcome: "route", note: "Partner Travel Agency 220 seats short — covenant-affecting, must route to the planner who owns the desk." } },
    { id: "C", name: "Balanced — serve most agencies", sub: "Spread capacity to minimise total shortfall.",
      ...
      pac: { rule: "PROM-covenant-agency", outcome: "route", note: "Partner Travel Agency 220 seats short — covenant-affecting, routed to the planner." } },
  ];

  const PAC_TONE = { allow: { l: "PAC · allow", cls: "allow", ic: "checkmark-filled" }, route: { l: "PAC · route to planner", cls: "route", ic: "group" } };

  function gbp(n) { return "£" + (n / 1e6).toFixed(2) + "M"; }
  ...
  function ScreenUDP({ toast, setScreen, naphtha = {} }) {
    ...
    React.useEffect(() => {
      if (sampleCount > seenSamples.current) {
        ...
        setFeed(f => [{ g: "jaz-mirabel", region: "IT", qty: 10 + Math.round(Math.random() * 16), src: "Hold · " + (s.grade || "Travel Assistant"), t: "just now", live: true }, ...f].slice(0, 8));
        setPulse(p => p + 1);
      }
    }, [sampleCount]);
    ...
    return (
      <div>
        ...
        <div className="udp-kpis">
          ...
          <Kpi label="Rotations constrained" value="1 of 2" sub="LHR → SSH over capacity · LGW → SSH healthy" tone="warn" />
          <Kpi label="Revenue at risk" value={gbp(revAtRisk) + "/wk"} sub={`${shortfall.toLocaleString()} seats of demand unservable as-is`} tone="warn" live />
              ...
              return (
                <button key={o.id} className={`udp-opt ${sel === o.id ? "on" : ""} ${o.covenant === "held" ? "safe" : "risk"}`} onClick={() => setSel(o.id)}>
                  ...
                  <div className="udp-opt-row">
                    ...
                    <span className="udp-opt-rev">{gbp(oi.rev)}/wk</span>
                  </div>
                </button>
              );
            })}
          ...
          <div className="udp-impact-wrap">
            ...
            <div className="udp-impact">
              <div className="udp-imp-grid">
                <div className="udp-imp"><span className="udp-imp-k">Revenue / wk</span><span className="udp-imp-v">{gbp(imp.rev)}</span></div>
              ...
              <div className={`udp-imp-cov ${opt.covenant === "held" ? "ok" : "risk"}`}>
                ...
                <span>{opt.covenant === "held" ? "agency covenant held — contracted Jaz Mirabel seats covered in full." : "agency covenant at risk — contracted Jaz Mirabel seats short by 220."}</span>
              </div>
        ...
        {committed && opt.covenant === "held" && (
          <div className="udp-exec">
            ...
            <div className="udp-exec-writes">
              {[["analytics", "Rotation schedule", "LHR → SSH rotation locked to the split below"], ["document-chart", "EasyBook order book", "Bookings confirmed or re-pointed per agency"], ["recommend", "Dynamic pricing", "Per-package prices set to seat tightness"]].map((w, i) => (
              ...
              {accounts.map((a, i) => (
                <div className="udp-blast-row" key={i}>
                  ...
                  <span className={`ub-st ${a.short === 0 ? "full" : a.covenant ? "held" : "short"}`}>
                    ...
                    {a.short === 0 ? "Served in full" : `${a.short.toLocaleString()} seats → LGW rotation next week · notified`}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="udp-exec routed">
            <div className="udp-exec-h">
              ...
              <div className="udp-exec-ht">
                ...
                <div className="udp-exec-s">{opt.name} shorts Partner Travel Agency's contracted seats by 220. That's covenant-affecting, so the rotation is held for human sign-off.</div>
              </div>
              <span className="udp-exec-trace">PLN-2026-0412 · {opt.pac.rule} → route</span>
            </div>
            <div className="udp-exec-writes">
              {[["document-chart", "Evidence bundle", "Cause · impact · all three options · the covenant breach"], ["lightbulb", "Recommended alternative", "Option A — protect covenant — attached"], ["group", "Owner", "Duty planner · Partner Travel Agency desk"]].map((w, i) => (
