    ...
    build(id) {
      ...
      if (id === "reg") {
        return {
          ...
          decision: { tone: "hold", label: "Held · propagated", verdict: "Answers and entry rules update; any formal entry confirmation is held for the trade desk.", body: "Travel Assistant answers and the package selector adjust immediately; an entry-bundle refresh is queued and impact briefs go to the affected agencies.", points: [{ icon: "checkmark-filled", text: "Travel Assistant + selector updated" }, { icon: "warning-alt", text: "Formal entry document → trade desk" }], rule: "TRV-consular-boundary · UC6" },
        };
      }
      // outage (hero)
      return {
        ...
        stages: [
          [{ tag: "BOOKING", t: "BK 88412 · Family A", x: "3 pax · Jaz Mirabel · departs 14 Aug" }, { tag: "ALLOTMENT", t: "Family TravelClub guaranteed block", x: "rooms held · same class & amenities" }],
          ...
          [{ tag: "PROM", t: "PROM-covenant-agency", x: "Re-accommodation must hold class + amenities." }, { tag: "TRV", t: "TRV-advisory-EG", x: "Same corridor, advisory ≤ 2 — no new assessment." }],
        ],
        decision: { tone: "act", label: "Action committed", verdict: "Re-pointed to the guaranteed block — the Family A 14 Aug dates hold, same room class and amenities.", body: "The covenant is preserved and the re-point is in bounds, so the spine commits it and the planner confirms the covenant-affecting step.", points: [{ icon: "checkmark-filled", text: "Room class + amenities preserved · dates held" }, { icon: "group", text: "Planner confirmed the covenant step" }], rule: "PROM-covenant-agency · Playbook §7.1" },
      };
    },
  };
    ...
    build(id) {
      if (id === "pack") {
        return {
          ...
          stages: [
            [{ tag: "PREMIUM TRAVEL", t: "Santorini for two", x: "Caldera suite · best fit" }, { t: "Bali wellness retreat", x: "Long-haul alternative" }],
      ...
      if (id === "foam") {
        return {
          ...
          stages: [
            [{ tag: "SIGNATURE TRAVEL", t: "Japan Signature Journey", x: "10 nights · best fit" }, { t: "Andalusia at ease", x: "Closer-to-home alternative" }],
      ...
      return {
        ...
        stages: [
          [{ tag: "FAMILY TRAVEL", t: "Jaz Mirabel Beach", x: "Shaded baby pool · best fit" }, { t: "Coral Bay", x: "Strong kids' club · smaller pool" }, { t: "Crete last minute", x: "budget alternative" }],
          [{ tag: "FCT", t: "Baby pool + kids' club", x: "leads the Sharm programme" }, { tag: "ENT", t: "EG e-visa + passports", x: "handled · ≥ 6 months valid" }],
          [{ t: "Entry rules", x: "EG entry clear for the party." }, { t: "Availability", x: "Family TravelClub guaranteed block holds the rooms." }, { t: "Policy", x: "Covenant room class + amenities preserved." }],
        ],
    ...
    return (
      <div className="dcg-reco-band">
        <div className="dcg-reco-eyebrow">
          <span className="dcg-reco-pill"><span className="dow-spark">✦</span> Travel Assistant · how it recommends</span>
          <h2>One package, chosen for you — and the reason it won.</h2>
          <p>Describe the trip and Travel Assistant reads it into a brief, matches packages, weighs the evidence, clears the rules, and returns one recommendation. Pick a need to see it resolve.</p>
        </div>
