...
(function () {
    ...
    return (
      <div>
        ...
        <div className="dg-legend">
          {[["customer", "Agency", "var(--k-spruce-40)"], ["sku", "Package / departure", "#E0CDF2"], ["plant", "Allotment", "var(--k-blue-50)"], ["cracker", "Resort wing / contract", "var(--k-warm-red-30)"], ["lims", "Proposal / fit check", "var(--k-status-success-20)"], ["policy", "Covenant / policy", "#F0C674"]].map((l, i) => (
    ...
    return (
      <div className="lpt">
        <div className="lpt-head">
          <Eyebrow>Live event path · EVT-2026-0810</Eyebrow>
          <div className="dash-chart-sub">The allotment disruption, traced state-by-state through the compiled journey. Each step shows the control value that unlocked the next transition.</div>
        </div>
  ...
  function GraphViews({ toast, view, setView, mapRef }) {
    ...
    const CFG = {
      ...
      governance: { data: M.governance, radius: 40, dense: false, src: "TravelRadar ↔ PAC policy lifecycle" },
    };
    const cfg = CFG[view];
    return (
  ...
  function UploadGate({ onApply, toast }) {
    ...
    const SEED = [
      ...
      { name: "ALP-SOP-TO-EU-v3.0.pdf", size: "540 KB", kind: "PDF Document", at: "Superseded · 3 Mar 2026", status: "Superseded", tone: "gray" },
      { name: "ALP-SOP-WINTER-v1.2.pdf", size: "612 KB", kind: "PDF Document", at: "Signed 28 Jan 2026", status: "Signed off", tone: "green" },
      { name: "Travel Assistant_B2B_Orchestration_Blueprint.docx", size: "103 KB", kind: "Word document", at: "Draft · 11 May 2026", status: "Draft", tone: "violet" },
    ];
    ...
    function addFile(name, size) {
      ...
      toast(`Added ${name} to the playbook library — review or apply it.`);
    }
    function apply(name) {
      ...
      <div className="disc">
        ...
        <div className="disc-cards">
          <div className="disc-card">
            ...
            <div className="disc-c-b">Upload your signed playbook / SOP PDFs to generate the target operating model and executable rules.</div>
            <button className="dow-btn red" onClick={pick} style={{ background: "var(--k-spruce-60)" }}><Icon name="arrow-up-right" size={15} /> Upload Operating Playbook</button>
          </div>
          <div className="disc-card">
            <div className="disc-c-eye">02 · Execute policy</div>
            <div className="disc-c-b">Apply a tour-operations playbook to compile Policy as Code and bind the agents to their decision rights.</div>
            <button className="dow-btn ghost" onClick={() => apply()}>{applyingName ? "Applying…" : "Apply current playbook →"}</button>
          </div>
        </div>
        ...
        <div className="disc-lib">
          <div className="disc-lib-top">
            <div>
              <div className="disc-c-eye">Operating playbook library</div>
              <div className="disc-lib-h">{files.length} document{files.length === 1 ? "" : "s"} · apply one to compile</div>
              <div className="disc-lib-s">Previously uploaded playbooks. Apply the signed current version, or upload a new one to add it to the library.</div>
            </div>
  ...
  function BrdParse({ runId }) {
    ...
    return (
      <div className="bx">
        <div className="spine-eyebrow" style={{ marginTop: 26 }}><span className="se-l">One document → every consequence</span><span className="se-r">The same signed playbook drives the per-country policy exceptions, the runtime decision, and the audit trail. <strong style={{ color: "var(--k-spruce-70)" }}>Watch it parse from a single artefact.</strong></span></div>
            ...
            <div className="bx-lanes">
              ...
              <div className={`bx-lane ${stage >= 2 ? "" : "pending"}`}>
                <div className="bx-lane-head">
                  ...
                  <span className="bx-h">The rule fires — with the playbook line attached</span>
                ...
                {stage >= 2 && (
                  <div className="bx-stream grid-2" style={{ gridTemplateColumns: "1fr 1fr", alignItems: "start" }}>
                    ...
                    <div className="panel" style={{ padding: 14, background: "#FBF9FE", border: "1px solid #E8DCF5" }}>
                      <span className="bx-eye" style={{ color: "#8A4FBF" }}>The source rule · in the playbook</span>
              ...
              <div className={`bx-lane ${stage >= 3 ? "" : "pending"}`}>
                <div className="bx-lane-head">
                  ...
                  <span className="bx-h">Every decision, traced to its playbook section</span>
    ...
    return (
      <div>
            ...
            {compiled ? (
              <React.Fragment>
                <div className="bd-pages">
                  ...
                  <div className="bd-sec">§4 Room & seat positioning</div>
              ...
              <div className="panel-load">
                <span className="pl-label">Reading Travel Group's operating rules</span>
          ...
          <div>
            ...
            <div className="brd2-cta">
              ...
              <div style={{ flex: 1, fontSize: 12.5, color: "var(--fg-1)", lineHeight: 1.5 }}>
                {compiled
                  ? <span><strong>Spine configured from the playbook.</strong> 198 policies live · 9 entity types resolved · 4 agents bound within their decision rights.</span>
        ...
        <div className={`brd2-out ${compiled ? "" : "hidden"}`}>
          <div className="spine-eyebrow" style={{ marginTop: 26 }}><span className="se-l">What the playbook compiled into</span><span className="se-r">Business activities, the group ontology, and decision rights — straight from the document. <strong style={{ color: "var(--k-spruce-70)" }}>Open any one in the graph below.</strong></span></div>
          ...
          <div className="spine-eyebrow" style={{ marginTop: 26 }}><span className="se-l">The Decision Graph</span><span className="se-r">Toggle through what the playbook compiled — the group ontology, the business activities, who may decide what, and the live event path.</span></div>
          <GraphViews toast={toast} view={graphView} setView={setGraphView} mapRef={graphRef} />

          {/* LIMS lineage */}
          <div style={{ marginTop: 26 }}>
            <Collapsible title="Proposal lineage" subtitle="Proposals, holds and fit checks are first-class entities — the re-accommodation journey, not a portal dead-end.">
