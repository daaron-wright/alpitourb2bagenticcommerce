    ...
    const items = React.useMemo(() => {
      ...
      if (!secured) {
        list.push({ id: "NBA-1", sev: "high", lens: "ops", screen: "flow", cta: "Review & sign off",
          title: "Sign off the re-accommodation", why: "3 booking writes are staged and reversible. The Family A 14-Aug date holds only once you release them — the allotment agent is paused on your decision." });
      } else {
        list.push({ id: "NBA-1b", sev: "info", lens: "ops", screen: "flow", cta: "View the sealed flow",
          title: "Re-accommodation committed", why: acked ? "Signed off and Partner Travel Agency accepted the secured plan — audit sealed. Nothing further needed." : "Signed off and committed. Awaiting Partner Travel Agency's confirmation on the customer side." });
      }
      list.push({ id: "NBA-2", sev: "high", lens: "ops", screen: "backtest", cta: "Open backtest lab",
        title: "Resolve policy conflict CFL-014", why: "Draft bundle v3.2 strands the re-point — covenant vs margin floor. Route to Compliance + Finance before it can promote past gate 3." });
      if (samples.length) {
        list.push({ id: "NBA-S", sev: "med", lens: "ops", screen: "queue", cta: "Open planner queue",
          title: `Confirm ${samples.length} Travel Assistant hold${samples.length > 1 ? "s" : ""}`, why: `Partner Travel Agency requested ${samples.map(s => s.grade).join(", ")} via Travel Assistant — added to BK 88412 and waiting on a covenant check.` });
      }
      list.push({ id: "NBA-3", sev: "med", lens: "ops", screen: "learning", cta: "Open inbox",
        title: "Close 2 high-severity knowledge gaps", why: "Unanswered questions are blocking confident agency answers — route them to the concierge and trade desks." });
      list.push({ id: "NBA-4", sev: "med", lens: "ops", screen: "backtest", cta: "View deployment gates",
        title: "Gate 4 promotion is blocked", why: "Audit completeness is below 100%. Close it to unlock agency-visible commitments." });
      if (secured && !acked) {
        list.push({ id: "NBA-5", sev: "info", lens: "cx", screen: "customer", cta: "See customer view",
          title: "Partner Travel Agency hasn't confirmed yet", why: "The re-point is committed and the dates hold — nudge the agency to accept the secured plan from the customer lens." });
      }
